...
Partial Class new_approve_resign_3ee36c046351
    ...
    Protected Sub cmd_confirm_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmd_confirm.Click
        ...
        parameter(3) = New OracleParameter("canrem", OracleType.VarChar, 150)
        ...
        oh.ExecuteNonQuery("m_resigning_can", parameter)
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
            ...
            If frm = 24 Or frm = 16 Or frm = 31 Or frm = 6 Then
                Server.Transfer("Cancel_resign_p.aspx")
                Exit Sub
            End If
            ...
            If dt.Rows(0)(0) = 378 Then
                ...
                sql = "select r.emp_code || ' --- ' || e.emp_name || ' --- Branch: ' || b.branch_name, e.emp_code from m_resign_appl r, employee_master e, branch b, employ_firm f where e.emp_code = r.emp_code and e.EMP_CODE = f.emp_code and f.firm_id = " & ff & " and e.branch_id = b.branch_id and r.status in (6,0) and  e.post_id in 195 and e.status_id = 1 order by emp_code"
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    ' Dim dt2 As DataTable = oh.ExecuteDataSet("select to_date(r.enter_dt),r.resign_dt, u.categ || ' -- ' || w.college_nm || ' , ' || w.course || ' , ' || w.durtion as reason, to_date(r.relieve_dt) from m_resign_appl r, resign_reason_mst u, resign_higherstudies_reason w where r.emp_code = " & Me.cmb_emp.SelectedValue & " and r.reason = u.categ_id and w.emp_code = r.emp_code and to_date(r.resign_dt) = to_date(w.tra_dt) and w.status = r.status and r.status in (6,0) union select to_date(r.enter_dt),r.resign_dt, u.categ || ' -- ' || q.reason as reason, to_date(r.relieve_dt) from m_resign_appl r, resign_reason_mst u, resign_personal_reason q where u.categ_id = r.reason and r.emp_code = " & Me.cmb_emp.SelectedValue & " and q.emp_code = r.emp_code and to_date(r.resign_dt) = to_date(q.tra_dt) and q.status = r.status and r.status in (6,0) union select to_date(r.enter_dt),r.resign_dt, u.categ || ' -- ' || q1.firm || ' , ' || q1.reason || ' , ' || q1.nature_work || ' , ' || q1.salary as reason, to_date(r.relieve_dt) from m_resign_appl r, resign_reason_mst u, resign_otheremploy_reason q1 where r.emp_code = " & Me.cmb_emp.SelectedValue & " and r.reason = u.categ_id and q1.emp_code = r.emp_code and to_date(r.resign_dt) = to_date(q1.tra_dt) and q1.status = r.status and r.status in (6,0) union select to_date(r.enter_dt),r.resign_dt, u.categ || ' -- ' || q.reason as reason, to_date(r.relieve_dt) from m_resign_appl r, resign_reason_mst u, resign_personal_reason q where r.emp_code = " & Me.cmb_emp.SelectedValue & " and u.categ_id = r.reason and q.emp_code = r.emp_code and to_date(r.resign_dt) = to_date(q.tra_dt) and q.status = r.status and r.status in (6,0) union select to_date(r.enter_dt),r.resign_dt, u.categ || ' -- ' || q2.partner_name || ' , ' || q2.job_partner || ' , ' || q2.place_marriage as reason, to_date(r.relieve_dt) from m_resign_appl r, resign_reason_mst u, resign_marriage_reason q2 where r.emp_code = " & Me.cmb_emp.SelectedValue & " and r.reason = u.categ_id and q2.emp_code = r.emp_code and to_date(r.resign_dt) = to_date(q2.tra_dt) and q2.status = r.status and r.status in (6,0) union select to_date(r.enter_dt),r.resign_dt, u.categ || ' -- ' || q3.reason as reason, to_date(r.relieve_dt) from m_resign_appl r, resign_reason_mst u, resign_other_reason q3 where r.emp_code = " & Me.cmb_emp.SelectedValue & " and u.categ_id = r.reason and q3.emp_code = r.emp_code and to_date(r.resign_dt) = to_date(q3.tra_dt) and q3.status = r.status and r.status in (6,0)").Tables(0)
                    Dim dt2 As DataTable = oh.ExecuteDataSet("select to_date(r.enter_dt), r.resign_dt, u.categ || ' -- ' || w.college_nm || ' , ' || w.course || ' , ' || w.durtion as reason, to_date(r.relieve_dt) from m_resign_appl r, resign_reason_mst u, resign_higherstudies_reason w where r.emp_code = " & Me.cmb_emp.SelectedValue & " and r.reason = u.categ_id and w.emp_code = r.emp_code and to_date(w.tra_dt)= (select max(q2.tra_dt) from resign_higherstudies_reason q2 where q2.emp_code=r.emp_code) and w.status = r.status and r.status in (6, 0) union select to_date(r.enter_dt), r.resign_dt, u.categ || ' -- ' || q.reason as reason, to_date(r.relieve_dt) from m_resign_appl r, resign_reason_mst u, resign_personal_reason q where u.categ_id = r.reason and r.emp_code = " & Me.cmb_emp.SelectedValue & " and q.emp_code = r.emp_code and to_date(q.tra_dt)= (select max(q2.tra_dt) from resign_personal_reason q2 where q2.emp_code=r.emp_code) and q.status = r.status and r.status in (6, 0) union select to_date(r.enter_dt), r.resign_dt, u.categ || ' -- ' || q1.firm || ' , ' || q1.reason || ' , ' || q1.nature_work || ' , ' || q1.salary as reason, to_date(r.relieve_dt) from m_resign_appl r, resign_reason_mst u, resign_otheremploy_reason q1 where r.emp_code = " & Me.cmb_emp.SelectedValue & " and r.reason = u.categ_id and q1.emp_code = r.emp_code and to_date(q1.tra_dt)= (select max(q2.tra_dt) from resign_otheremploy_reason q2 where q2.emp_code=r.emp_code) and q1.status = r.status and r.status in (6, 0) union select to_date(r.enter_dt), r.resign_dt, u.categ || ' -- ' || q.reason as reason, to_date(r.relieve_dt) from m_resign_appl r, resign_reason_mst u, resign_personal_reason q where r.emp_code = " & Me.cmb_emp.SelectedValue & " and u.categ_id = r.reason and q.emp_code = r.emp_code and to_date(q.tra_dt)=(select max(q2.tra_dt) from resign_personal_reason q2 where q2.emp_code=r.emp_code) and q.status = r.status and r.status in (6, 0) union select to_date(r.enter_dt), r.resign_dt, u.categ || ' -- ' || q2.partner_name || ' , ' || q2.job_partner || ' , ' || q2.place_marriage as reason, to_date(r.relieve_dt) from m_resign_appl r, resign_reason_mst u, resign_marriage_reason q2 where r.emp_code = " & Me.cmb_emp.SelectedValue & " and r.reason = u.categ_id and q2.emp_code = r.emp_code and to_date(q2.tra_dt)=(select max(q2.tra_dt) from resign_marriage_reason q2 where q2.emp_code=r.emp_code) and q2.status = r.status and r.status in (6, 0) union select to_date(r.enter_dt), r.resign_dt, u.categ || ' -- ' || q3.reason as reason, to_date(r.relieve_dt) from m_resign_appl r, resign_reason_mst u, resign_other_reason q3 where r.emp_code = " & Me.cmb_emp.SelectedValue & " and u.categ_id = r.reason and q3.emp_code = r.emp_code and to_date(q3.tra_dt)=(select max(q2.tra_dt) from resign_other_reason q2 where q2.emp_code=r.emp_code) and q3.status = r.status and r.status in (6, 0)").Tables(0)
                    Me.lbl_code.Text = dt1.Rows(0)(0)
                    Me.lbl_name.Text = dt1.Rows(0)(1)
                    Me.Labelapp.Text = Format(CDate(dt2.Rows(0)(0)), "dd/MMM/yyyy")
                    Me.Txt_rsdt.Text = Format(CDate(dt2.Rows(0)(1)), "dd/MMM/yyyy")
                    Me.Txt_rdt.Text = Format(CDate(dt2.Rows(0)(1)), "dd/MMM/yyyy")
            ...
            ElseIf dt.Rows(0)(0) = 85 Or dt.Rows(0)(0) = 195 Or dt.Rows(1)(1) = 1 Then
                ...
                sql = "select r.emp_code||' --- '||e.emp_name||'  ---  Branch: '||b.branch_name,e.emp_code from m_resign_appl r,employee_master e,branch b,employ_firm f where e.emp_code=r.emp_code and e.EMP_CODE=f.emp_code and f.firm_id=" & ff & " and e.branch_id=b.branch_id and r.status in (6,0) and e.status_id=1 order by emp_code"
                dt = oh.ExecuteDataSet(sql).Tables(0)
                Me.cmb_emp.DataSource = dt
                Me.cmb_emp.DataTextField = dt.Columns(0).ColumnName
                Me.cmb_emp.DataValueField = dt.Columns(1).ColumnName
            ...
            Else
                ...
                Dim dt3 As DataTable = oh.ExecuteDataSet("select post_id,branch_id from employee_master where emp_code=" & usr(0) & " and status_id=1 ").Tables(0)
                If dt3.Rows.Count > 0 Then
                    ...
                    If dt.Rows.Count > 0 Then
                        ...
                        Me.Labelapp.Text = Format(CDate(dt2.Rows(0)(0)), "dd/MMM/yyyy")
                        Me.Txt_rsdt.Text = Format(CDate(dt2.Rows(0)(1)), "dd/MMM/yyyy")
                        If IsDBNull(dt2.Rows(0)(3)) Then
                            Me.Txt_rdt.Text = ""
                        Else
                            Me.Txt_rdt.Text = Format(CDate(dt2.Rows(0)(3)), "dd/MMM/yyyy")
                        End If
    ...
    Protected Sub cmb_emp_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmb_emp.SelectedIndexChanged
        ...
        Dim dt1 As DataTable = oh.ExecuteDataSet("select emp_code,emp_name from employee_master where emp_code=" & Me.cmb_emp.SelectedValue & " and status_id=1 ").Tables(0)
        'Dim dt2 As DataTable = oh.ExecuteDataSet("select to_date(r.enter_dt),to_date(r.resign_dt),u.categ||' -- '||w.college_nm||' , '||w.course||' , '||w.durtion as reason,to_date(r.relieve_dt) from m_resign_appl r,resign_reason_mst u,resign_higherstudies_reason w where  r.emp_code=" & Me.cmb_emp.SelectedValue & " and r.reason=u.categ_id and w.emp_code=r.emp_code and to_date(r.resign_dt)=to_date(w.tra_dt) and w.status=r.status and r.status in (6,0) union select to_date(r.enter_dt),to_date(r.resign_dt),u.categ||' -- '||q.reason as reason,to_date(r.relieve_dt) from m_resign_appl r,resign_reason_mst u,resign_personal_reason q where u.categ_id=r.reason and r.emp_code=" & Me.cmb_emp.SelectedValue & " and q.emp_code=r.emp_code and to_date(r.resign_dt)=to_date(q.tra_dt) and q.status=r.status and r.status in (6,0) union select to_date(r.enter_dt),to_date(r.resign_dt),u.categ||' -- '||q1.firm||' , '||q1.reason||' , '||q1.nature_work||' , '||q1.salary as reason,to_date(r.relieve_dt) from m_resign_appl r,resign_reason_mst u,resign_otheremploy_reason q1 where  r.emp_code=" & Me.cmb_emp.SelectedValue & " and r.reason=u.categ_id and q1.emp_code=r.emp_code and to_date(r.resign_dt)=to_date(q1.tra_dt) and q1.status=r.status and r.status in (6,0) union select to_date(r.enter_dt),to_date(r.resign_dt),u.categ||' -- '||q.reason as reason,to_date(r.relieve_dt) from m_resign_appl r,resign_reason_mst u,resign_personal_reason q where  r.emp_code=" & Me.cmb_emp.SelectedValue & " and u.categ_id=r.reason and q.emp_code=r.emp_code and to_date(r.resign_dt)=to_date(q.tra_dt) and q.status=r.status and r.status in (0,6) union select to_date(r.enter_dt),to_date(r.resign_dt),u.categ||' -- '||q2.partner_name||' , '||q2.job_partner||' , '||q2.place_marriage as reason,to_date(r.relieve_dt) from m_resign_appl r,resign_reason_mst u,resign_marriage_reason q2 where  r.emp_code=" & Me.cmb_emp.SelectedValue & " and r.reason=u.categ_id and q2.emp_code=r.emp_code and to_date(r.resign_dt)=to_date(q2.tra_dt) and q2.status=r.status and r.status in (0,6) union select to_date(r.enter_dt),to_date(r.resign_dt),u.categ||' -- '||q3.reason as reason,to_date(r.relieve_dt) from m_resign_appl r,resign_reason_mst u,resign_other_reason q3 where  r.emp_code=" & Me.cmb_emp.SelectedValue & " and u.categ_id=r.reason and q3.emp_code=r.emp_code and to_date(r.resign_dt)=to_date(q3.tra_dt) and q3.status=r.status and r.status in (6,0) ").Tables(0)
        Dim dt2 As DataTable = oh.ExecuteDataSet("select to_date(r.enter_dt), r.resign_dt, u.categ || ' -- ' || w.college_nm || ' , ' || w.course || ' , ' || w.durtion as reason, to_date(r.relieve_dt) from m_resign_appl r, resign_reason_mst u, resign_higherstudies_reason w where r.emp_code = " & Me.cmb_emp.SelectedValue & " and r.reason = u.categ_id and w.emp_code = r.emp_code and to_date(w.tra_dt)= (select max(q2.tra_dt) from resign_higherstudies_reason q2 where q2.emp_code=r.emp_code) and w.status = r.status and r.status in (6, 0) union select to_date(r.enter_dt), r.resign_dt, u.categ || ' -- ' || q.reason as reason, to_date(r.relieve_dt) from m_resign_appl r, resign_reason_mst u, resign_personal_reason q where u.categ_id = r.reason and r.emp_code = " & Me.cmb_emp.SelectedValue & " and q.emp_code = r.emp_code and to_date(q.tra_dt)= (select max(q2.tra_dt) from resign_personal_reason q2 where q2.emp_code=r.emp_code) and q.status = r.status and r.status in (6, 0) union select to_date(r.enter_dt), r.resign_dt, u.categ || ' -- ' || q1.firm || ' , ' || q1.reason || ' , ' || q1.nature_work || ' , ' || q1.salary as reason, to_date(r.relieve_dt) from m_resign_appl r, resign_reason_mst u, resign_otheremploy_reason q1 where r.emp_code = " & Me.cmb_emp.SelectedValue & " and r.reason = u.categ_id and q1.emp_code = r.emp_code and to_date(q1.tra_dt)= (select max(q2.tra_dt) from resign_otheremploy_reason q2 where q2.emp_code=r.emp_code) and q1.status = r.status and r.status in (6, 0) union select to_date(r.enter_dt), r.resign_dt, u.categ || ' -- ' || q.reason as reason, to_date(r.relieve_dt) from m_resign_appl r, resign_reason_mst u, resign_personal_reason q where r.emp_code = " & Me.cmb_emp.SelectedValue & " and u.categ_id = r.reason and q.emp_code = r.emp_code and to_date(q.tra_dt)=(select max(q2.tra_dt) from resign_personal_reason q2 where q2.emp_code=r.emp_code) and q.status = r.status and r.status in (6, 0) union select to_date(r.enter_dt), r.resign_dt, u.categ || ' -- ' || q2.partner_name || ' , ' || q2.job_partner || ' , ' || q2.place_marriage as reason, to_date(r.relieve_dt) from m_resign_appl r, resign_reason_mst u, resign_marriage_reason q2 where r.emp_code = " & Me.cmb_emp.SelectedValue & " and r.reason = u.categ_id and q2.emp_code = r.emp_code and to_date(q2.tra_dt)=(select max(q2.tra_dt) from resign_marriage_reason q2 where q2.emp_code=r.emp_code) and q2.status = r.status and r.status in (6, 0) union select to_date(r.enter_dt), r.resign_dt, u.categ || ' -- ' || q3.reason as reason, to_date(r.relieve_dt) from m_resign_appl r, resign_reason_mst u, resign_other_reason q3 where r.emp_code = " & Me.cmb_emp.SelectedValue & " and u.categ_id = r.reason and q3.emp_code = r.emp_code and to_date(q3.tra_dt)=(select max(q2.tra_dt) from resign_other_reason q2 where q2.emp_code=r.emp_code) and q3.status = r.status and r.status in (6, 0)").Tables(0)
        Me.lbl_code.Text = dt1.Rows(0)(0)
        Me.lbl_name.Text = dt1.Rows(0)(1)
        Me.Txt_rsdt.Text = Format(CDate(dt2.Rows(0)(1)), "dd/MMM/yyyy")
    ...
    Protected Sub Button1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        End If
        Dim sql2 As String
        'sql2 = "select t.status as stat from m_resign_appl t where t.emp_code= " & Me.lbl_code.Text & " "
        sql2 = "select t.status as stat from m_resign_appl t where t.emp_code =" & Me.lbl_code.Text & " and t.status<>3"
        ...
        If dt2.Rows(0)(0) = 0 Then
            ...
            parameter(3) = New OracleParameter("rejrem", OracleType.VarChar, 150)
            parameter(3).Direction = ParameterDirection.Input
            parameter(3).Value = Me.Text_remar.Text
            ...
            parameter(4) = New OracleParameter("msg", OracleType.Number, 150)
            parameter(4).Direction = ParameterDirection.Output
            oh.ExecuteNonQuery("M_RESIGNING_REJ", parameter)
            If parameter(4).Value = 1 Then
                Dim cl_script1 As New System.Text.StringBuilder
        ...
        ElseIf dt2.Rows(0)(0) = 6 Then
            ...
            parameter(4) = New OracleParameter("msg", OracleType.Number, 150)
            parameter(4).Direction = ParameterDirection.Output
            oh.ExecuteNonQuery("M_RESIGNING_REJ_2", parameter)
            If parameter(4).Value = 1 Then
                Dim cl_script1 As New System.Text.StringBuilder
                cl_script1.Append("        alert('Rejected successfully!!');")
                cl_script1.Append("window.open('cancel_resign.aspx','_self');")
        ...
        Else
            Dim cl_script1 As New System.Text.StringBuilder
            cl_script1.Append("        alert('Contact IT Department...!!');")
            ' cl_script1.Append("window.open('cancel_resign.aspx','_self');")
            Page.ClientScript.RegisterClientScriptBlock(Me.GetType, "client script", cl_script1.ToString, True)
            Exit Sub
        End If
