...
Partial Class Leave_Module_rpt_leave_applied_status_c717618e3714
    Inherits System.Web.UI.Page
    Dim oh As New helper.oracle.oraclehelper
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Dim str As String = "select e.leave_frdate,e.leave_todate,e.leave_apply_date,e.leave_days,lm.leave_abbr,m.category_name,d.reason_name,decode(e.status_id,1,'SANCTIONED',0,'APPLIED',2,'REJECTED',3,'CANCELLED',4,'RECOMMENDED') as status,case when e.recom_person is not null then (select emp_code||' - ' || emp_name from employee_master where emp_code=e.recom_person) end as rec_person,e.recom_date,e.reject_reason,case when e.sanc_person is not null then (select emp_code||' - ' || emp_name from employee_master where emp_code=e.sanc_person) end as sanc_person,e.sanc_date from hrm_leave_apply_sanction e,hrm_category_master m,hrm_category_dtl d ,leave_master lm where e.emp_code=" & Me.Request.QueryString("empcode") & " and e.leave_frdate between('" & Request.QueryString("fromdt") & "') and ('" & Request.QueryString("todt") & "') and e.category_id=m.category_id and m.category_id=d.category_id and e.leave_id=lm.leave_id  and e.reason_id=d.reason_id order by e.leave_frdate"
        ...
        tabc2.Text = "<body align=center color=red><b><font size=3.5> LEAVE STATUS REPORT </font></b></body>"
