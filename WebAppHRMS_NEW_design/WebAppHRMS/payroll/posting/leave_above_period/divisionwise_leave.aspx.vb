...
Partial Class leave_above_10_divisionwise_leave_3b6002e13475
    ...
    Dim desig As Integer = 0
    Dim dpost As Integer = 0
    Dim dleave As Integer = 0
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Me.rid = Me.Request.QueryString("regionid")
        Me.desig = Me.Request.QueryString("designation")
        Me.dpost = Me.Request.QueryString("post")
        Me.dleave = Me.Request.QueryString("leaveno")

        Dim Div_table As New Table
        Div_table.Attributes.Add("width", "100%")
        ...
        Div_table.Controls.Add(sheader)
        ...
        pheadercell.Text = "<body align=center ><b><font size=3> Divisionwise list of employees having  " & Me.Request.QueryString("leaveno") & " or more leaves </font></b>"
        pheader.Controls.Add(pheadercell)
        Div_table.Controls.Add(pheader)
        ...
        Div_table.Controls.Add(pheaderq)
        ...
        If Me.Request.QueryString("status") = 0 Then
            ...
            If Me.Request.QueryString("designation") <> 0 Then
                ...
                str = "select c.division_id,c.DIV_NAME,count(a.emp_code) from employee_master a,branch_detail c where a.status_id not in (3,5) and a.designation_id=" & Me.Request.QueryString("designation") & " and a.emp_code in (select emp_code from employ_leave_dtl where to_date(leave_frdate)>=to_date('1-jan-'||to_char(sysdate,'yyyy')) and leave_process_id in (1,2) and status=1 having sum(leave_days)>=" & Me.Request.QueryString("leaveno") & " group by emp_code)and a.branch_id=c.BRANCH_ID and c.reg_id=" & Me.Request.QueryString("regionid") & " group by c.division_id,c.DIV_NAME"

            ElseIf Me.Request.QueryString("post") <> 0 Then

                str = "select c.division_id,c.DIV_NAME,count(a.emp_code) from employee_master a,branch_detail c where a.status_id not in (3,5) and a.post_id=" & Me.Request.QueryString("post") & " and a.emp_code in (select emp_code from employ_leave_dtl where to_date(leave_frdate)>=to_date('1-jan-'||to_char(sysdate,'yyyy')) and leave_process_id in (1,2) and status=1 having sum(leave_days)>=" & Me.Request.QueryString("leaveno") & " group by emp_code)and a.branch_id=c.BRANCH_ID and c.reg_id=" & Me.Request.QueryString("regionid") & " group by c.division_id,c.DIV_NAME"

            Else
                str = "select c.division_id,c.DIV_NAME,count(a.emp_code) from employee_master a,branch_detail c where a.status_id not in (3,5) and a.emp_code in (select emp_code from employ_leave_dtl where to_date(leave_frdate)>=to_date('1-jan-'||to_char(sysdate,'yyyy')) and leave_process_id in (1,2) and status=1 having sum(leave_days)>=" & Me.Request.QueryString("leaveno") & " group by emp_code)and a.branch_id=c.BRANCH_ID and c.reg_id=" & Me.Request.QueryString("regionid") & " group by c.division_id,c.DIV_NAME"
        ...
        ElseIf Me.Request.QueryString("status") = 1 Then
            ...
            If Me.Request.QueryString("designation") <> 0 Then
                ...
                str = "select c.division_id,c.DIV_NAME,count(a.emp_code) from employee_master a,branch_detail c where a.status_id not in (3,5) and a.emp_type=1 and a.designation_id=" & Me.Request.QueryString("designation") & " and a.emp_code in (select emp_code from employ_leave_dtl where to_date(leave_frdate)>=to_date('1-jan-'||to_char(sysdate,'yyyy')) and leave_process_id in (1,2) and status=1 having sum(leave_days)>=" & Me.Request.QueryString("leaveno") & " group by emp_code)and a.branch_id=c.BRANCH_ID and c.reg_id=" & Me.Request.QueryString("regionid") & " group by c.division_id,c.DIV_NAME"

            ElseIf Me.Request.QueryString("post") <> 0 Then

                str = "select c.division_id,c.DIV_NAME,count(a.emp_code) from employee_master a,branch_detail c where a.status_id not in (3,5) and a.emp_type=1 and a.post_id=" & Me.Request.QueryString("post") & " and a.emp_code in (select emp_code from employ_leave_dtl where to_date(leave_frdate)>=to_date('1-jan-'||to_char(sysdate,'yyyy')) and leave_process_id in (1,2) and status=1 having sum(leave_days)>=" & Me.Request.QueryString("leaveno") & " group by emp_code)and a.branch_id=c.BRANCH_ID and c.reg_id=" & Me.Request.QueryString("regionid") & " group by c.division_id,c.DIV_NAME"

            Else
                str = "select c.division_id,c.DIV_NAME,count(a.emp_code) from employee_master a,branch_detail c where a.status_id not in (3,5) and a.emp_type=1 and a.emp_code in (select emp_code from employ_leave_dtl where to_date(leave_frdate)>=to_date('1-jan-'||to_char(sysdate,'yyyy')) and leave_process_id in (1,2) and status=1 having sum(leave_days)>=" & Me.Request.QueryString("leaveno") & " group by emp_code)and a.branch_id=c.BRANCH_ID and c.reg_id=" & Me.Request.QueryString("regionid") & " group by c.division_id,c.DIV_NAME"
        ...
        ElseIf Me.Request.QueryString("status") = 2 Then
            ...
            If Me.Request.QueryString("designation") <> 0 Then
                ...
                str = "select c.division_id,c.DIV_NAME,count(a.emp_code) from employee_master a,branch_detail c where a.status_id not in (3,5) and a.emp_type=2 and a.designation_id=" & Me.Request.QueryString("designation") & " and a.emp_code in (select emp_code from employ_leave_dtl where to_date(leave_frdate)>=to_date('1-jan-'||to_char(sysdate,'yyyy')) and leave_process_id in (1,2) and status=1 having sum(leave_days)>=" & Me.Request.QueryString("leaveno") & " group by emp_code)and a.branch_id=c.BRANCH_ID and c.reg_id=" & Me.Request.QueryString("regionid") & " group by c.division_id,c.DIV_NAME"

            ElseIf Me.Request.QueryString("post") <> 0 Then

                str = "select c.division_id,c.DIV_NAME,count(a.emp_code) from employee_master a,branch_detail c where a.status_id not in (3,5) and a.emp_type=2 and a.post_id=" & Me.Request.QueryString("post") & " and a.emp_code in (select emp_code from employ_leave_dtl where to_date(leave_frdate)>=to_date('1-jan-'||to_char(sysdate,'yyyy')) and leave_process_id in (1,2) and status=1 having sum(leave_days)>=" & Me.Request.QueryString("leaveno") & " group by emp_code)and a.branch_id=c.BRANCH_ID and c.reg_id=" & Me.Request.QueryString("regionid") & " group by c.division_id,c.DIV_NAME"

            Else
                str = "select c.division_id,c.DIV_NAME,count(a.emp_code) from employee_master a,branch_detail c where a.status_id not in (3,5) and a.emp_type=2 and a.emp_code in (select emp_code from employ_leave_dtl where to_date(leave_frdate)>=to_date('1-jan-'||to_char(sysdate,'yyyy')) and leave_process_id in (1,2) and status=1 having sum(leave_days)>=" & Me.Request.QueryString("leaveno") & " group by emp_code)and a.branch_id=c.BRANCH_ID and c.reg_id=" & Me.Request.QueryString("regionid") & " group by c.division_id,c.DIV_NAME"
        ...
        f2.Text = "<b><font size=2>Division Name</font></b>"
        ...
        For Each dr In dt.Rows
            ...
            '"<a href=divisionwise_leave.aspx?status=" & Me.Request.QueryString("status") & "&regionid=" & dr(0) & "&designation=" & Me.edes & "&post=" & Me.epost & "&leaveno=" & Me.Request.QueryString("leaveno") & "><font size=2>" & dr(1) & "</font></a>"
            v1.Text = "<a href=areawise_leave.aspx?status=" & Me.Request.QueryString("status") & "&divid=" & dr(0) & "&designation=" & Me.Request.QueryString("designation") & "&post=" & Me.Request.QueryString("post") & "&leaveno=" & Me.Request.QueryString("leaveno") & "><font size=2>" & dr(1) & "</font></a>"
            val.Controls.Add(v1)
        ...
        Div_table.Controls.Add(totrow)


        Panel_Div.Controls.Add(Div_table)
