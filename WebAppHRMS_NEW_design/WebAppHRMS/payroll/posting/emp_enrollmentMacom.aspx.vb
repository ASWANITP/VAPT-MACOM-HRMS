...
Partial Class payroll_posting_emp_enrollment_ab6c5ba75923
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Session("access_id") = 33 Then
            If Not IsPostBack Then
                ...
                dt2 = oh.ExecuteDataSet("select distinct d.dep_name, d.dep_id from mactech.department_mst d,mactech.employ_firm ef,mactech.employee_master em where em.department_id = d.dep_id and em.emp_code = ef.emp_code and ef.firm_id = 8 and em.status_id = 1 order by d.dep_name asc").Tables(0)



                Me.cmb_dep.DataSource = dt2
                Me.cmb_dep.DataTextField = dt2.Columns(0).ColumnName
                Me.cmb_dep.DataValueField = dt2.Columns(1).ColumnName
                Me.cmb_dep.DataBind()
                basic_pay()
                ...
                If frm = 16 Or frm = 33 Then
                    dt33 = oh.ExecuteDataSet("select -1,'---------Select----------' from dual union all select 0 emp_code, 'No Recommendation' from dual union all select e.emp_code, e.emp_code||'--'||e.emp_name   from mactech.employee_master  e  join mactech.employ_firm f on f.emp_code=e.emp_code and f.firm_id in (16,33) where e.status_id=1  order by 1 ").Tables(0)
                    dt44 = oh.ExecuteDataSet("select -1,'---------Select----------' from dual union all select e.emp_code, e.emp_code||'--'||e.emp_name   from mactech.employee_master  e  join mactech.employ_firm f on f.emp_code=e.emp_code and f.firm_id in (16,33) where e.status_id=1  order by 1 ").Tables(0)
                ElseIf frm = 6 Or frm = 14 Or frm = 31 Or frm = 32 Then
                    dt33 = oh.ExecuteDataSet("select -1,'---------Select----------' from dual union all select 0 emp_code, 'No Recommendation' from dual union all select e.emp_code, e.emp_code||'--'||e.emp_name   from mactech.employee_master  e  join mactech.employ_firm f on f.emp_code=e.emp_code and f.firm_id in (6,14,31,32) where e.status_id=1  order by 1 ").Tables(0)
                    dt44 = oh.ExecuteDataSet("select -1,'---------Select----------' from dual union all select e.emp_code, e.emp_code||'--'||e.emp_name   from mactech.employee_master  e  join mactech.employ_firm f on f.emp_code=e.emp_code and f.firm_id in (6,14,31,32) where e.status_id=1  order by 1 ").Tables(0)
                Else
                    dt33 = oh.ExecuteDataSet("select -1,'---------Select----------' from dual union all select 0 emp_code, 'No Recommendation' from dual union all select e.emp_code, e.emp_code||'--'||e.emp_name   from mactech.employee_master  e  join mactech.employ_firm f on f.emp_code=e.emp_code and f.firm_id=" & frm & " where e.status_id=1  order by 1 ").Tables(0)
                    dt44 = oh.ExecuteDataSet("select -1,'---------Select----------' from dual union all select e.emp_code, e.emp_code||'--'||e.emp_name   from mactech.employee_master  e  join mactech.employ_firm f on f.emp_code=e.emp_code and f.firm_id=" & frm & " where e.status_id=1  order by 1 ").Tables(0)
                End If
