...
Public Class Shift_Sanction
    Inherits System.Web.UI.Page
    Dim dt, dt1, dt2, dt3, dt4, dt5 As New DataTable
    Dim sf() As String
    Private Const REJECT_FLAG As Integer = 1  ' Fixed forever
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
            ...
            Else
                ...
                masterPage.Subtitle = " TEAM LEAD SHIFT CHANGE RECOMMEND "


                dt4 = oh.ExecuteDataSet("select -1 as request_id, '-----Select-----' as sname, to_date(null) as request_date from dual union all select distinct t.request_id, e.emp_name || ' --> ' || e.emp_code || '-->' || t.req_dt|| '-->' || 'Req_ID:' || t.request_id, t.req_dt  from mactech.MACOM_SHIFT_CHANGE t, mactech.employee_master e, mactech.tl_trsfr_level k where t.status = 1 and t.emp_code = k.emp_code and k.tl_empcode = " & sf(0) & " and t.emp_code = e.emp_code order by request_id").Tables(0)
    ...
    Protected Sub btnRecommend_Click(sender As Object, e As EventArgs) Handles btnRecommend.Click
        ...
        DateTime.TryParse(Me.txtEffectiveDate.Text, effectiveDate)
        If effectiveDate <= DateTime.Now.Date Then
            script2.Append("alert('Please Reject Request :Effective Date must be before today');")
        ...
        parameter(1) = New OracleParameter("sanc_empid", OracleType.Number, 150)
        parameter(1).Direction = ParameterDirection.Input
        parameter(1).Value = sf(0)

        parameter(2) = New OracleParameter("remark", OracleType.VarChar, 250)
        ...
        Try
            ...
            script1.Append("window.open('Shift_Sanction.aspx','_self');")
    ...
    Protected Sub btnReject_Click(sender As Object, e As EventArgs) Handles btnReject.Click
        ...
        parameter(6) = New OracleParameter("msg", OracleType.VarChar, 500)
    ...
    Protected Sub ddlEmployee_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ddlEmployee.SelectedIndexChanged
        ...
        If ddlEmployee.SelectedValue <> "-1" Then
            ...
            Dim sql As String = "select e.emp_name, e.emp_code, d.dep_name, ds.designation, b.branch_name, tl.tl_empcode, m.eff_dt, td.shift || ' --> ' || td.in_time || ' -- ' || td.out_time, tn.shift || ' --> ' || tn.in_time || ' -- ' || tn.out_time from employee_master e inner join department_mst d on e.department_id = d.dep_id inner join designation_master ds on e.designation_id = ds.designation_id inner join post_mst p on e.post_id = p.post_id inner join branch_master b on e.branch_id = b.branch_id inner join tl_trsfr_level tl on e.emp_code = tl.emp_code inner join MACOM_SHIFT_CHANGE m on m.emp_code = e.emp_code and m.request_id = :tlEmpCode inner join time_tab td on m.old_shift_id = td.shift_id inner join time_tab tn on m.new_shift_id = tn.shift_id"
            ...
            Dim p As New OracleParameter("tlEmpCode", OracleType.Number) With {.Value = tlId}
            ...
            Me.txtOldShift.Text = dt5.Rows(0)(7)
            Me.ddlRequestedShift.Text = dt5.Rows(0)(8)
            Me.txtEffectiveDate.Text = dt5.Rows(0)(6)
