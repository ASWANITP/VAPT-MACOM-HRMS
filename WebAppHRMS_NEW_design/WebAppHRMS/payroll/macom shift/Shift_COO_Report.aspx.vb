...
Public Class Shift_COO_Report
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            '' Execute and bind
            '  Dim sql As String = "select e.emp_name, e.emp_code, d.dep_name, ds.designation, b.branch_name, tl.tl_empcode, m.eff_dt, m.req_dt, td.shift || ' --> ' || td.in_time || ' -- ' || td.out_time as old_shift, tn.shift || ' --> ' || tn.in_time || ' -- ' || tn.out_time as new_shift, m.emp_remark, m.tl_remark, m.dephed_remark, m.coo_remark, m.reject_code, m.reject_remark, m.status, case m.status when 0 then 'Rejected' when 1 then 'Applied' when 2 then 'TL Recomended' when 3 then 'Dep Head Recomended' when 4 then 'Approved' else '---' end as status_text from employee_master e inner join department_mst d on e.department_id = d.dep_id inner join designation_master ds on e.designation_id = ds.designation_id inner join post_mst p on e.post_id = p.post_id inner join branch_master b on e.branch_id = b.branch_id inner join tl_trsfr_level tl on e.emp_code = tl.emp_code inner join MACOM_SHIFT_CHANGE m on m.emp_code = e.emp_code and m.status in(4,0)  and m.req_dt between :FromDate and :ToDate inner join time_tab td on m.old_shift_id = td.shift_id inner join time_tab tn on m.new_shift_id = tn.shift_id order by m.request_id"
            Dim sql As String = "select e.emp_name, e.emp_code, d.dep_name,  m.eff_dt, m.req_dt, td.shift || ' --> ' || td.in_time || ' -- ' || td.out_time as old_shift, tn.shift || ' --> ' || tn.in_time || ' -- ' || tn.out_time as new_shift, m.emp_remark, m.tl_remark, m.dephed_remark, m.coo_remark, m.reject_code, m.reject_remark, m.status, case m.status when 0 then 'Rejected' when 1 then 'Applied' when 2 then 'TL Recomended' when 3 then 'Dep Head Recomended' when 4 then 'Approved' else '---' end as status_text from employee_master e inner join department_mst d on e.department_id = d.dep_id inner join designation_master ds on e.designation_id = ds.designation_id inner join post_mst p on e.post_id = p.post_id inner join branch_master b on e.branch_id = b.branch_id inner join tl_trsfr_level tl on e.emp_code = tl.emp_code inner join MACOM_SHIFT_CHANGE m on m.emp_code = e.emp_code and (m.status =4 or m.reject_code=:EmpCode) and m.req_dt between :FromDate and :ToDate inner join time_tab td on m.old_shift_id = td.shift_id inner join time_tab tn on m.new_shift_id = tn.shift_id order by m.request_id"
