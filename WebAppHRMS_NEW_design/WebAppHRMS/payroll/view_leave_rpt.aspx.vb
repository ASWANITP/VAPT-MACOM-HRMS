...
Partial Class Leave_Details_view_leave_rpt_33e25c207021
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        pheadercell.Text = "<body align=center ><b><font size=3> Leave Details of EmpCode:" & Me.Request.QueryString("empcode") & " between " & Me.Request.QueryString("leavefrom") & " and " & Me.Request.QueryString("leaveto") & " </font></b>"
        ...
        ElseIf Me.Request.QueryString("leavetype") = 4 Then
            ...
            'ElseIf Me.Request.QueryString("leavetype") = 6 Then
            '    pheadercellq.Text = "<body align=center ><b><font size=3>Leave Type:: Long Leave</font></b>"
            'ElseIf Me.Request.QueryString("leavetype") = 10 Then
            '    pheadercellq.Text = "<body align=center ><b><font size=3>Leave Type:: Maternity</font></b>"
        End If
        ...
        'line1.Width = 6
        'linecell1.ColumnSpan = 6
        'linecell1.Text = "<hr>"
        'line1.Controls.Add(linecell1)
        'leavedtlstable.Controls.Add(line1)


        If Me.Request.QueryString("leavetype") = 0 Then
            '                 0            1             2               3              4            5                6
            str = "select em.emp_code,em.emp_name,el.leave_frdate,el.leave_todate,nvl(el.leave_days,0),el.leave_reason,case when el.leave_id=4 and el.leave_process_id=1 then 'Sal. Not Processed (L.O.P)' when el.leave_id=4 and el.leave_process_id=2 then 'Sal. Processed (L.O.P)'when el.leave_id=4 and el.leave_process_id=3 then 'Entered For Arrear Salary (L.O.P)'else '-----' end as Leave_Status from employ_leave_dtl el,employee_master em where em.emp_code=el.emp_code and el.leave_process_id<>0 and el.leave_id in(1,2,3,4) and em.emp_code=" & Me.Request.QueryString("empcode") & " and to_date(el.leave_frdate)>=to_date('" & Me.Request.QueryString("leavefrom") & "') and to_date(el.leave_frdate)<=to_date('" & Me.Request.QueryString("leaveto") & "')"
        Else
            str = "select em.emp_code,em.emp_name,el.leave_frdate,el.leave_todate,nvl(el.leave_days,0),el.leave_reason,case when el.leave_id=4 and el.leave_process_id=1 then 'Sal. Not Processed (L.O.P)' when el.leave_id=4 and el.leave_process_id=2 then 'Sal. Processed (L.O.P)'when el.leave_id=4 and el.leave_process_id=3 then 'Entered For Arrear Salary (L.O.P)'else '-----' end as Leave_Status from employ_leave_dtl el,employee_master em where em.emp_code=el.emp_code and el.leave_process_id<>0 and el.leave_id=" & Me.Request.QueryString("leavetype") & " and em.emp_code=" & Me.Request.QueryString("empcode") & " and to_date(el.leave_frdate)>=to_date('" & Me.Request.QueryString("leavefrom") & "') and to_date(el.leave_frdate)<=to_date('" & Me.Request.QueryString("leaveto") & "')"
        End If
        ...
        Else
            Dim ecode As New TableRow
            Dim ecodek As New TableCell
            ecode.Width = 6
            ecodek.ColumnSpan = 6
            ecodek.HorizontalAlign = HorizontalAlign.Left
            ecodek.Text = "<font size=3><b>Employee Code:</b>" & dt.Rows(0)(0) & "</font>"
            ecode.Controls.Add(ecodek)
            leavedtlstable.Controls.Add(ecode)

            Dim enamer As New TableRow
            Dim enamerk As New TableCell
            enamer.Width = 6
            enamerk.ColumnSpan = 6
            enamerk.HorizontalAlign = HorizontalAlign.Left
            enamerk.Text = "<font size=3><b>Employee Name:</b>" & dt.Rows(0)(1) & "</font>"
            enamer.Controls.Add(enamerk)
            leavedtlstable.Controls.Add(enamer)
