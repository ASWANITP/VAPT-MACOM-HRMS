...
Partial Class payroll_Hrm_Add_Post_Macom_090990d38136
        ...
        Select Case (DataStr(1))
            ...
            Case 2
                If (DataStr(0) = "-33") Then
                    ...
                    dt = oh.ExecuteDataSet("select 0, '---POST---' as postname from dual union select p.post_id, p.post_name as postname from post_mst p ,employee_master e ,employ_firm f where p.post_id=e.post_id and e.emp_code=f.emp_code and f.firm_id=8 and e.emp_code>9999 order by postname").Tables(0)
                ...
                ElseIf (DataStr(0) = "-44") Then
                    ...
                    dt = oh.ExecuteDataSet("select 0, '---DESIGNATION---' as desig from dual union select d.designation_id, d.designation as desig from designation_master d,employee_master e ,employ_firm f where d.designation_id=e.designation_id and e.emp_code=f.emp_code and f.firm_id=8 and e.emp_code>9999 order by desig").Tables(0)
                    CbResult = FillData(CbResult, dt)
                    CbResult = CbResult + "@"
                ElseIf (DataStr(0) = "-55") Then
                    ...
                    dt = oh.ExecuteDataSet("select 0, '---DEPARTMENT---' as dep from dual union select d.dep_id, d.dep_name as dep from department_mst d ,employee_master e ,employ_firm f where d.dep_id=e.department_id and e.emp_code=f.emp_code and f.firm_id=8 and e.emp_code>9999 order by dep").Tables(0)
                    CbResult = FillData(CbResult, dt)
                    CbResult = CbResult + "@"
                ElseIf (DataStr(0) = "-66") Then
                    dt = oh.ExecuteDataSet("select 0, '---TECHLEAD---' as techlead from dual union select em.emp_code, em.emp_name from employee_master em, employ_Firm f where f.emp_code = em.emp_Code and f.firm_id = 8 and em.status_id = 1").Tables(0)
                    CbResult = FillData(CbResult, dt)
                    CbResult = CbResult + "@"
