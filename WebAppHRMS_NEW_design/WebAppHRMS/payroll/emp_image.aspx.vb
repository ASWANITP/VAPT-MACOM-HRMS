...
Partial Class emp_image_emp_image_c9c6bd786985
    ...
    ''MODIFIED JEWEL REQUEST KRISHNADAS

    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        User = Me.Session("user_id").ToString.Split("!")
        usr = user(0)
        Me.hid3.value = usr
        ...
        If Not IsPostBack Then
            Me.hid2.Value = 0
            int3 = int4 = 0
            Dim sql As String = "select count(*) from macdms.hrm_emp_ph_certi where emp_code=" & usr
            Dim stabssle As Integer = oh.ExecuteDataSet(sql).Tables(0).Rows(0)(0)
            If stabssle > 0 Then
                sql = "select qc.category,qm.qualification,e.qualification,e.marks,e.total,e.photo,e.certificate from macdms.hrm_emp_ph_certi e,qualification_master qm,qualification_category qc where emp_code=" & usr & " and e.qualification=qm.qualification_id and qm.category_id=qc.category_id"
    ...
    Protected Sub cmd_confirm_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmd_confirm.Click
        ...
        Else
            Dim cl_script As New StringBuilder
            cl_script.Append("   alert('NO FILE FOUND') ;")
            Page.ClientScript.RegisterClientScriptBlock(Me.GetType, "inv", cl_script.ToString, True)
            Exit Sub
        End If
        ...
        Try
            ...
            If int3 = 0 And int4 = 0 Then
                ...
                If int1 = 1 And int2 = 1 Then
                    sql = "insert into macdms.hrm_emp_ph_certi(emp_code,qualification,marks,total,photo,certificate)  values(:code,:quali,:mark,:tot,:ph,:certi)"
                ElseIf int1 = 1 Then
                    sql = "insert into macdms.hrm_emp_ph_certi(emp_code,qualification,marks,total,photo)  values(:code,:quali,:mark,:tot,:ph)"
                Else
                    sql = "insert into macdms.hrm_emp_ph_certi(emp_code,qualification,marks,total,certificate)  values(:code,:quali,:mark,:tot,:certi)"
                End If
            ...
            ElseIf int3 = 1 And int4 = 0 Then
                sql = "update macdms.hrm_emp_ph_certi h set h.certificate=:certi where h.emp_code=:code"
            ...
            ElseIf int3 = 0 And int4 = 1 Then
                sql = "update macdms.hrm_emp_ph_certi h set h.photo= :ph where h.emp_code=:code"
                Dim parm_coll1(1) As OracleParameter
