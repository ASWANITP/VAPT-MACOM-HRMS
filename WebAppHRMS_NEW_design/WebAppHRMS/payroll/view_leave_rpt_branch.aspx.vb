...
Partial Class Leave_Details_view_leave_rpt_33e25c202684
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Me.Request.QueryString("leavetype") = 0 Then
            '                 0            1             2               3              4            5                6
            str = "select em.emp_code,em.emp_name,el.leave_frdate,el.leave_todate,nvl(el.leave_days,0),el.leave_reason,case when el.leave_id=4 and el.leave_process_id=1 then 'Sal. Not Processed (L.O.P)' when el.leave_id=4 and el.leave_process_id=2 then 'Sal. Processed (L.O.P)'when el.leave_id=4 and el.leave_process_id=3 then 'Entered For Arrear Salary (L.O.P)'else '-----' end as Leave_Status from employ_leave_dtl el,employee_master em where em.emp_code=el.emp_code and el.leave_process_id<>0 and el.leave_id in(1,2,3,4) and  em.firm_id=" & Session("firm_id") & " and em.branch_id=" & Me.Request.QueryString("branchid") & " and to_date(el.leave_frdate)>=to_date('" & Me.Request.QueryString("leavefrom") & "') and to_date(el.leave_frdate)<=to_date('" & Me.Request.QueryString("leaveto") & "')"

        Else
            str = "select em.emp_code,em.emp_name,el.leave_frdate,el.leave_todate,nvl(el.leave_days,0),el.leave_reason,case when el.leave_id=4 and el.leave_process_id=1 then 'Sal. Not Processed (L.O.P)' when el.leave_id=4 and el.leave_process_id=2 then 'Sal. Processed (L.O.P)'when el.leave_id=4 and el.leave_process_id=3 then 'Entered For Arrear Salary (L.O.P)'else '-----' end as Leave_Status from employ_leave_dtl el,employee_master em where em.emp_code=el.emp_code and el.leave_process_id<>0 and  em.firm_id=" & Session("firm_id") & " and em.branch_id=" & Me.Request.QueryString("branchid") & "  and el.leave_id=" & Me.Request.QueryString("leavetype") & " and to_date(el.leave_frdate)>=to_date('" & Me.Request.QueryString("leavefrom") & "') and to_date(el.leave_frdate)<=to_date('" & Me.Request.QueryString("leaveto") & "')"
        End If
