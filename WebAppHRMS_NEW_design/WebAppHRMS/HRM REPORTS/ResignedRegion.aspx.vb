...
Partial Class HRM_ResignedRegion_389af18d1416
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack() Then
            ...
            If Status = 1 Then
                dt = oh.ExecuteDataSet("select d.reg_id,d.reg_name,count(*) as count from employee_master a,employee_master_dtl b,examination_dtl c,branch_detail d where d.zonal_id=" & ZonalID & "  and a.status_id=3 and a.emp_code=b.emp_code and b.discont_dt is not null and to_date(b.discont_dt)>='" & FromDt & "' and to_date(b.discont_dt)<='" & ToDt & "' and c.status='P' and a.emp_code=c.emp_code and b.emp_code=c.emp_code and a.branch_id=d.branch_id group by d.reg_id,d.reg_name").Tables(0)
            Else
                dt = oh.ExecuteDataSet("select z.reg_id,z.reg_name,count(*) as count from (select c.reg_id,c.reg_name from  employee_master a,employee_master_dtl b,branch_detail c where c.zonal_id=" & ZonalID & " and a.emp_code=b.emp_code and a.status_id=3 and b.discont_dt is not null and to_date(b.discont_dt)>='" & FromDt & "' and to_date(b.discont_dt)<='" & ToDt & "' and a.branch_id=c.branch_id and a.emp_code not in (select emp_code from examination_dtl) union all select c.reg_id,c.reg_name from  employee_master a,employee_master_dtl b,branch_detail c where c.zonal_id=" & ZonalID & " and a.emp_code=b.emp_code and a.status_id=3 and b.discont_dt is not null and to_date(b.discont_dt)>='" & FromDt & "' and to_date(b.discont_dt)<='" & ToDt & "' and a.branch_id=c.branch_id and a.emp_code in (select emp_code from examination_dtl where status='F')) z group by z.reg_id,z.reg_name").Tables(0)
            End If
    ...
    Public Sub Head_print()
        ...
        If Status = 1 Then
            tr05_01.Text = "REGIONWISE TRAINED RESIGNED EMPLOYEES REPORT "
        Else
            tr05_01.Text = "REGIONWISE UNTRAINED RESIGNED EMPLOYEES REPORT "
        End If
