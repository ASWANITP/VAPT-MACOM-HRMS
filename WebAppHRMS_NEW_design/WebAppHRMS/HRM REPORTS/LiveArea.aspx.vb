...
Partial Class HRM_ResignedArea_5a7fd99d7447
    Inherits System.Web.UI.Page
    Dim oh As New helper.oracle.OracleHelper
    Dim dt As New DataTable
    Dim tb As New Table
    Dim dr As DataRow
    Dim FromDt, ToDt, DivisionName As String
    Dim DivisionID, tot_training, tot_participant, Status As Integer
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack() Then
            FromDt = Request.QueryString.Get("FromDt")
            ToDt = Request.QueryString.Get("ToDt")
            DivisionID = Request.QueryString.Get("DivisionID")
            Status = Request.QueryString.Get("Status")
            dt = oh.ExecuteDataSet("select div_name from division_master where division_id=" & DivisionID & "").Tables(0)
            DivisionName = dt.Rows(0)(0)
            Dim sql As String = ""
            If Status = 1 Then
                'sql = "select d.area_id,d.area_name,count(*) as count from employee_master a,examination_dtl c,branch_detail d where d.division_id=" & DivisionID & "  and a.status_id=1 and to_date(a.join_dt)>='" & FromDt & "' and to_date(a.join_dt)<='" & ToDt & "' and c.status='P' and a.emp_code=c.emp_code and a.branch_id=d.branch_id group by d.area_id,d.area_name"
                sql = "select d.area_id,d.area_name,count(*) as count from employee_master a,examination_dtl c,branch_detail d where d.division_id=" & DivisionID & "  and a.status_id=1 and c.status='P' and a.emp_code=c.emp_code and a.branch_id=d.branch_id group by d.area_id,d.area_name"
                dt = oh.ExecuteDataSet(sql).Tables(0)
            Else
                'sql = "select z.area_id,z.area_name,count(*) as count from (select c.area_id,c.area_name from  employee_master a,branch_detail c where c.division_id=" & DivisionID & " and a.status_id=1 and to_date(a.join_dt)>='" & FromDt & "' and to_date(a.join_dt)<='" & ToDt & "' and a.branch_id=c.branch_id and a.emp_code not in (select emp_code from examination_dtl) union all select c.area_id,c.area_name from  employee_master a,branch_detail c where c.division_id=" & DivisionID & " and a.status_id=1 and to_date(a.join_dt)>='" & FromDt & "' and to_date(a.join_dt)<='" & ToDt & "' and a.branch_id=c.branch_id and a.emp_code in (select emp_code from examination_dtl where status='F')) z group by z.area_id,z.area_name"
                sql = "select z.area_id,z.area_name,count(*) as count from (select c.area_id,c.area_name from  employee_master a,branch_detail c where c.division_id=" & DivisionID & " and a.status_id=1  and a.branch_id=c.branch_id and a.emp_code not in (select emp_code from examination_dtl) union all select c.area_id,c.area_name from  employee_master a,branch_detail c where c.division_id=" & DivisionID & " and a.status_id=1 and to_date(a.join_dt)>='" & FromDt & "' and to_date(a.join_dt)<='" & ToDt & "' and a.branch_id=c.branch_id and a.emp_code in (select emp_code from examination_dtl where status='F')) z group by z.area_id,z.area_name"
                dt = oh.ExecuteDataSet(sql).Tables(0)
            End If
    ...
    Public Sub Head_print()
        ...
        If Status = 1 Then
            tr05_01.Text = "AREAWISE TRAINED EXISTING EMPLOYEES REPORT "
        Else
            tr05_01.Text = "AREAWISE UNTRAINED EXISTING EMPLOYEES REPORT "
        End If
