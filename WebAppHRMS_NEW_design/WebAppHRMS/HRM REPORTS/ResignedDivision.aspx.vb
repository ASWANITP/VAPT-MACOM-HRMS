...
Partial Class HRM_ResignedDivision_839af5254964
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack() Then
            ...
            If Status = 1 Then
                dt = oh.ExecuteDataSet("select d.division_id,d.div_name,count(*) as count from employee_master a,employee_master_dtl b,examination_dtl c,branch_detail d where d.reg_id=" & RegionID & "  and a.status_id=3 and a.emp_code=b.emp_code and b.discont_dt is not null and to_date(b.discont_dt)>='" & FromDt & "' and to_date(b.discont_dt)<='" & ToDt & "' and c.status='P' and a.emp_code=c.emp_code and b.emp_code=c.emp_code and a.branch_id=d.branch_id group by d.division_id,d.div_name").Tables(0)
            Else
                dt = oh.ExecuteDataSet("select z.division_id,z.div_name,count(*) as count from (select c.division_id,c.div_name from  employee_master a,employee_master_dtl b,branch_detail c where c.reg_id=" & RegionID & " and a.emp_code=b.emp_code and a.status_id=3 and b.discont_dt is not null and to_date(b.discont_dt)>='" & FromDt & "' and to_date(b.discont_dt)<='" & ToDt & "' and a.branch_id=c.branch_id and a.emp_code not in (select emp_code from examination_dtl) union all select c.division_id,c.div_name from  employee_master a,employee_master_dtl b,branch_detail c where c.reg_id=" & RegionID & " and a.emp_code=b.emp_code and a.status_id=3 and b.discont_dt is not null and to_date(b.discont_dt)>='" & FromDt & "' and to_date(b.discont_dt)<='" & ToDt & "' and a.branch_id=c.branch_id and a.emp_code in (select emp_code from examination_dtl where status='F')) z group by z.division_id,z.div_name").Tables(0)
            End If
    ...
    Public Sub Head_print()
        ...
        If Status = 1 Then
            tr05_01.Text = "DIVISIONWISE TRAINED RESIGNED EMPLOYEES REPORT "
        Else
            tr05_01.Text = "DIVISIONWISE UNTRAINED RESIGNED EMPLOYEES REPORT "
        End If
