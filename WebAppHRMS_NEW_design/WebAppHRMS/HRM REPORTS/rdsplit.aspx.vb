...
Partial Class salary_report_sal_wage_rpt_e471c8fb1658
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        dt = oh.ExecuteDataSet("select alls.emp_code,alls.name,alls.rd_amt,alls.month from (select m.emp_code, m.name, m.rdded_amt rd_amt, m.sal_dt sal, to_char(m.sal_dt,'MON/yyyy') month from m_wage m where m.emp_code = " & s & " union all select m.emp_code, m.name, m.rdded_amt rd_amt, m.sal_dt sal, to_char(m.sal_dt,'MON/yyyy') month from m_wage_his m where m.emp_code = " & s & " Order by sal) alls where alls.rd_amt > 0").Tables(0)
        If dt.Rows.Count < 0 Then
            If dt.Rows(0)(0) = 0 Then
                Dim cl_script As New StringBuilder
                cl_script.Append("   alert('RD Not Yet Processed.') ;")
        ...
        'user = Session("user_id").ToString.Split("!")
        'dt = oh.ExecuteDataSet("select alls.* from(select m.emp_code,m.name,m.rdded_amt rd_amt,to_char(m.sal_dt,'MON/yyyy')month from m_wage m where m.emp_code =" & s & " union all select m.emp_code,m.name,m.rdded_amt rd_amt,to_char(m.sal_dt,'MON/yyyy')month from m_wage_his m where m.emp_code =" & s & " )alls where alls.rd_amt>0").Tables(0)
        ...
        tabc2.Text = "<body align=center color=red><b><font size=3.5> RD SPLIT-UP OF EMPLOYEE " & s & "(" & dt1s.Rows(0)(0) & ") </font></b></body>"
        ...
        For Each dr In dt.Rows
            ...
            tabr5c1.Text = "<FONT SIZE=3>" & dr(3) & "</FONT>"
            tabr5c2.Text = "<FONT SIZE=3> " & dr(2) & "</FONT>"
