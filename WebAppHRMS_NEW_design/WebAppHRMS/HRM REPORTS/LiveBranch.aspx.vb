...
Partial Class HRM_ResignedBranch_e6c1e81e5386
    Inherits System.Web.UI.Page
    Dim oh As New helper.oracle.OracleHelper
    Dim dt As New DataTable
    Dim tb As New Table
    Dim dr As DataRow
    Dim FromDt, ToDt, AreaName As String
    Dim AreaID, Status, tot_participant As Integer
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack() Then
            ...
            dt = oh.ExecuteDataSet("select area_name from area_master where area_id=" & AreaID & "").Tables(0)
            AreaName = dt.Rows(0)(0)
            Dim sql As String = ""
            If Status = 1 Then
                'sql = "select d.branch_id,d.branch_name,count(*) as count from employee_master a,examination_dtl c,branch_detail d where d.area_id=" & AreaID & "  and a.status_id=1 and to_date(a.join_dt)>='" & FromDt & "' and to_date(a.join_dt)<='" & ToDt & "' and c.status='P' and a.emp_code=c.emp_code and a.branch_id=d.branch_id group by d.branch_id,d.branch_name"
                sql = "select d.branch_id,d.branch_name,count(*) as count from employee_master a,examination_dtl c,branch_detail d where d.area_id=" & AreaID & "  and a.status_id=1  and c.status='P' and a.emp_code=c.emp_code and a.branch_id=d.branch_id group by d.branch_id,d.branch_name"
                dt = oh.ExecuteDataSet(sql).Tables(0)
            Else
                'sql = "select z.branch_id,z.branch_name,count(*) as count from (select c.branch_id,c.branch_name from  employee_master a,branch_detail c where c.area_id=" & AreaID & " and a.status_id=1 and to_date(a.join_dt)>='" & FromDt & "' and to_date(a.join_dt)<='" & ToDt & "' and a.branch_id=c.branch_id and a.emp_code not in (select emp_code from examination_dtl) union all select c.branch_id,c.branch_name from  employee_master a,branch_detail c where c.area_id=" & AreaID & " and a.status_id=1 and to_date(a.join_dt)>='" & FromDt & "' and to_date(a.join_dt)<='" & ToDt & "' and a.branch_id=c.branch_id and a.emp_code in (select emp_code from examination_dtl where status='F')) z group by z.branch_id,z.branch_name"
                sql = "select z.branch_id,z.branch_name,count(*) as count from (select c.branch_id,c.branch_name from  employee_master a,branch_detail c where c.area_id=" & AreaID & " and a.status_id=1 and to_date(a.join_dt)>='" & FromDt & "' and to_date(a.join_dt)<='" & ToDt & "' and a.branch_id=c.branch_id and a.emp_code not in (select emp_code from examination_dtl) union all select c.branch_id,c.branch_name from  employee_master a,branch_detail c where c.area_id=" & AreaID & " and a.status_id=1 and a.branch_id=c.branch_id and a.emp_code in (select emp_code from examination_dtl where status='F')) z group by z.branch_id,z.branch_name"
                dt = oh.ExecuteDataSet(sql).Tables(0)
            End If
    ...
    Public Sub Head_print()
        ...
        If Status = 1 Then
            tr05_01.Text = "BRANCHWISE TRAINED EXISTING EMPLOYEES REPORT "
        Else
            tr05_01.Text = "BRANCHWISE UNTRAINED EXISTING EMPLOYEES REPORT "
        End If
