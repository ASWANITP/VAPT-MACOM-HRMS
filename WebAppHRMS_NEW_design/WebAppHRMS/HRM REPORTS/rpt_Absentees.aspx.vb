...
Imports System.Data
Partial Class HRM_Reports_rpt_Absentees_46296bec1157
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        'Dim QUERY As String = "select distinct td.training_id,td.topic,td.venue,tp.product_name from training_dtl td,training_products tp,training_attend ta where td.training_id>0 and td.training_from between '" & FromDt & "' and '" & ToDt & "' and td.training_id=ta.training_id and td.product_type = tp.product_type order by training_id"
        'Dim QUERY As String = "select em.emp_name,dm.designation,bm.branch_name,td.training_from,tp.product_name,td.venue,substr(td.trainer,0,11),(select nvl(comments,'NILL') from training_participant_dtl where status='17') as Informed,(select nvl(comments,'NILL') from training_participant_dtl where status='18') as NotInformed from training_participant_dtl tpd,employee_master em,designation_master dm, branch_master bm,training_dtl td,training_products tp where td.training_id>0 and td.training_from between '" & FromDt & "' and '" & ToDt & "' and tpd.status in ('17','18') and tpd.participant_id=em.emp_code and tpd.designation_id=dm.designation_id and tpd.branch_id=bm.branch_id and tpd.training_id=td.training_id and td.product_type=tp.product_type"
        'Dim QUERY As String = "select substr(em.emp_name,0,13),substr(pm.post_name,0,12),substr(bm.branch_name,0,12),td.training_from,substr(tp.product_name,0,12),substr(td.venue,0,12),substr(td.trainer,0,11),substr(decode(tpd.status,'17',tpd.comments,'Not Informed'),0,15) as Informed,em.emp_code from training_participant_dtl tpd,employee_master em,post_mst pm, branch_master bm,training_dtl td,training_products tp where td.training_id>0 and to_date(td.training_from) between '" & FromDt & "' and '" & ToDt & "' and tpd.status in ('17','18') and tpd.participant_id=em.emp_code and tpd.post_id=pm.post_id and tpd.branch_id=bm.branch_id and tpd.training_id=td.training_id and td.product_type=tp.product_type order by em.emp_name"
        Dim QUERY As String = "select substr(em.emp_name, 0, 13),substr(pm.post_name, 0, 12),substr(bm.branch_name, 0, 12),td.training_from,substr(tp.product_name, 0, 12),substr(td.venue, 0, 12),substr(td.trainer, 0, 11),substr(decode(tpd.status, '17', tpd.comments, 'Not Informed'), 0, 15) as Informed,em.emp_code from training_participant_dtl tpd,employee_master  em,post_mst   pm,branch_master  bm,training_dtl td,training_products  tp, employ_firm ef where td.training_id > 0  and to_date(td.training_from) between '" & FromDt & "' and '" & ToDt & "' and tpd.status in ('17', '18') and tpd.participant_id = em.emp_code and tpd.post_id = pm.post_id and tpd.branch_id = bm.branch_id and tpd.training_id = td.training_id and ef.emp_code=em.emp_code   and ef.firm_id=" & fir & " and td.product_type = tp.product_type and exists (select a.emp_code from hrm_tour_dtl a where a.emp_code = tpd.participant_id and a.tour_id = 2 and a.training_normal = 1 and (a.recom_person is null or a.reject_reason ='NOT ATTEND TRAINING') and to_date(a.from_dt) >= to_date('" & FromDt & "') and to_date(a.to_dt) <= to_date('" & ToDt & "')) order by em.emp_name"
