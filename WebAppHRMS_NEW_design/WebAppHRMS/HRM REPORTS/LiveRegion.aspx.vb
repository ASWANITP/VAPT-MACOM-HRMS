...
Partial Class HRM_LiveRegion_ba6d2a6c6250
    Inherits System.Web.UI.Page
    Dim oh As New helper.oracle.OracleHelper
    Dim dt As New DataTable
    Dim tb As New Table
    Dim dr As DataRow
    Dim FromDt, ToDt, ZonalName As String
    Dim ZonalID, tot_training, tot_participant, Status As Integer
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack() Then
            FromDt = Request.QueryString.Get("FromDt")
            ToDt = Request.QueryString.Get("ToDt")
            ZonalID = Request.QueryString.Get("ZonalID")
            Status = Request.QueryString.Get("Status")
            dt = oh.ExecuteDataSet("select zonal_name from zonal_master where zonal_id=" & ZonalID & "").Tables(0)
            ZonalName = dt.Rows(0)(0)
            Dim sql As String = ""
            If Status = 1 Then
                'sql = "select d.reg_id,d.reg_name,count(*) as count from employee_master a,examination_dtl c,branch_detail d where d.zonal_id=" & ZonalID & "  and a.status_id=1 and to_date(a.join_dt)>='" & FromDt & "' and to_date(a.join_dt)<='" & ToDt & "' and c.status='P' and a.emp_code=c.emp_code and a.branch_id=d.branch_id group by d.reg_id,d.reg_name"
                sql = "select d.reg_id,d.reg_name,count(*) as count from employee_master a,examination_dtl c,branch_detail d where d.zonal_id=" & ZonalID & "  and a.status_id=1  and c.status='P' and a.emp_code=c.emp_code and a.branch_id=d.branch_id group by d.reg_id,d.reg_name"
                dt = oh.ExecuteDataSet(sql).Tables(0)
            Else
                'sql = "select z.reg_id,z.reg_name,count(*) as count from (select c.reg_id,c.reg_name from  employee_master a,branch_detail c where c.zonal_id=" & ZonalID & " and a.status_id=1 and to_date(a.join_dt)>='" & FromDt & "' and to_date(a.join_dt)<='" & ToDt & "' and a.branch_id=c.branch_id and a.emp_code not in (select emp_code from examination_dtl) union all select c.reg_id,c.reg_name from  employee_master a,branch_detail c where c.zonal_id=" & ZonalID & " and a.status_id=1 and to_date(a.join_dt)>='" & FromDt & "' and to_date(a.join_dt)<='" & ToDt & "' and a.branch_id=c.branch_id and a.emp_code in (select emp_code from examination_dtl where status='F')) z group by z.reg_id,z.reg_name"
                sql = "select z.reg_id,z.reg_name,count(*) as count from (select c.reg_id,c.reg_name from  employee_master a,branch_detail c where c.zonal_id=" & ZonalID & " and a.status_id=1 and to_date(a.join_dt)>='" & FromDt & "' and to_date(a.join_dt)<='" & ToDt & "' and a.branch_id=c.branch_id and a.emp_code not in (select emp_code from examination_dtl) union all select c.reg_id,c.reg_name from  employee_master a,branch_detail c where c.zonal_id=" & ZonalID & " and a.status_id=1  and a.branch_id=c.branch_id and a.emp_code in (select emp_code from examination_dtl where status='F')) z group by z.reg_id,z.reg_name"
                dt = oh.ExecuteDataSet(sql).Tables(0)
            End If
    ...
    Public Sub Head_print()
        ...
        If Status = 1 Then
            tr05_01.Text = "REGIONWISE TRAINED EXISTING EMPLOYEES REPORT "
        Else
            tr05_01.Text = "REGIONWISE UNTRAINED EXISTING EMPLOYEES REPORT "
        End If
