...
Partial Class HRM_ResignedDivision_839af5253454
    Inherits System.Web.UI.Page
    Dim oh As New helper.oracle.OracleHelper
    Dim dt As New DataTable
    Dim tb As New Table
    Dim dr As DataRow
    Dim FromDt, ToDt, RegionName As String
    Dim RegionID, tot_training, tot_participant, Status As Integer
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack() Then
            FromDt = Request.QueryString.Get("FromDt")
            ToDt = Request.QueryString.Get("ToDt")
            RegionID = Request.QueryString.Get("RegionID")
            Status = Request.QueryString.Get("Status")
            dt = oh.ExecuteDataSet("select reg_name from region_master where reg_id=" & RegionID & "").Tables(0)
            RegionName = dt.Rows(0)(0)
            Dim sql As String = ""
            If Status = 1 Then
                'sql = "select d.division_id,d.div_name,count(*) as count from employee_master a,examination_dtl c,branch_detail d where d.reg_id=" & RegionID & "  and a.status_id=1 and to_date(a.join_dt)>='" & FromDt & "' and to_date(a.join_dt)<='" & ToDt & "' and c.status='P' and a.emp_code=c.emp_code and a.branch_id=d.branch_id group by d.division_id,d.div_name"
                sql = "select d.division_id,d.div_name,count(*) as count from employee_master a,examination_dtl c,branch_detail d where d.reg_id=" & RegionID & "  and a.status_id=1  and c.status='P' and a.emp_code=c.emp_code and a.branch_id=d.branch_id group by d.division_id,d.div_name"
                dt = oh.ExecuteDataSet(sql).Tables(0)
            Else
                'sql = "select z.division_id,z.div_name,count(*) as count from (select c.division_id,c.div_name from  employee_master a,branch_detail c where c.reg_id=" & RegionID & " and a.status_id=1 and to_date(a.join_dt)>='" & FromDt & "' and to_date(a.join_dt)<='" & ToDt & "' and a.branch_id=c.branch_id and a.emp_code not in (select emp_code from examination_dtl) union all select c.division_id,c.div_name from  employee_master a,branch_detail c where c.reg_id=" & RegionID & " and  a.status_id=1 and to_date(a.join_dt)>='" & FromDt & "' and to_date(a.join_dt)<='" & ToDt & "' and a.branch_id=c.branch_id and a.emp_code in (select emp_code from examination_dtl where status='F')) z group by z.division_id,z.div_name"
                sql = "select z.division_id,z.div_name,count(*) as count from (select c.division_id,c.div_name from  employee_master a,branch_detail c where c.reg_id=" & RegionID & " and a.status_id=1 and to_date(a.join_dt)>='" & FromDt & "' and to_date(a.join_dt)<='" & ToDt & "' and a.branch_id=c.branch_id and a.emp_code not in (select emp_code from examination_dtl) union all select c.division_id,c.div_name from  employee_master a,branch_detail c where c.reg_id=" & RegionID & " and  a.status_id=1 and a.branch_id=c.branch_id and a.emp_code in (select emp_code from examination_dtl where status='F')) z group by z.division_id,z.div_name"
                dt = oh.ExecuteDataSet(sql).Tables(0)
            End If
    ...
    Public Sub Head_print()
        ...
        If Status = 1 Then
            tr05_01.Text = "DIVISIONWISE TRAINED EXISTING EMPLOYEES REPORT "
        Else
            tr05_01.Text = "DIVISIONWISE UNTRAINED EXISTING EMPLOYEES REPORT "
        End If
