...
Partial Class Auction_Listed_pledges_ad40ce216603
    ...
    Sub FillColumnField()
        ...
        If brid = 0 Then
            ...
            For Each dr In dt.Rows
                ...
                tc56.Text = "<FONT SIZE =2><font color='blue'>" & FormatNumber(dr(5)) & "</font>"
        ...
        Else
            ...
            sql = "select rownum,xx.* from(select t.emp_code,  t.emp_name, to_char(t.join_dt), t.security_dep SECURITY_DEPOSIT,  nvl(sum(w.RDDED_AMT), 0) PAID_AMOUNT,  t.security_dep - nvl(sum(w.RDDED_AMT), 0) BALANCE_TO_BE_PAID,  decode(t.status_id, 1, 'LIVE', 'RESIGNED') Status,  case   when s.amount > 0 then   t.security_dep / s.amount   else   0  end NUMBER_OF_INSTALLMENTS from employee_master t join employ_firm f on f.emp_code = t.emp_code   and f.firm_id in ('" & Session("firm_id") & "') left join m_wage_all w on w.EMP_CODE = t.emp_code left join hrm_rd_security s on s.emp_code = t.emp_code where t.status_id='" & type & "' group by t.emp_code, t.emp_name,t.join_dt, t.security_dep, t.status_id, s.amount order by t.emp_code)xx"
            ...
            For Each dr In dt.Rows
                ...
                tc51.Text = "<FONT SIZE =2><font color='blue'>" & dr(0) & "</font>"
                tc52.Text = "<FONT SIZE =2><font color='blue'>" & dr(1) & "</font>"
                tc53.Text = "<FONT SIZE =2><font color='blue'>" & FormatNumber(dr(2)) & "</font>"
                tc54.Text = "<FONT SIZE =2><font color='blue'>" & FormatNumber(dr(3)) & "</font>"
                tc55.Text = "<FONT SIZE =2><font color='blue'>" & FormatNumber(dr(4)) & "</font>"
