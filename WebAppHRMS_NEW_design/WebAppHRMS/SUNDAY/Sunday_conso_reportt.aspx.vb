...
Partial Class SUNDAY_Sunday_conso_reportt_b6cf5efe6287
    Inherits System.Web.UI.Page
 
    Dim dt, dts As New DataTable
    Dim dr As DataRow
    Dim str, sql, strs, sf(), stft, sttt, frm As String
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        'If frm = 27 Then
        '    dts1 = oh.ExecuteDataSet("select query from mactech.hrm_report_master where firm_id=99 and query_id=146").Tables(0)
        ...
        Else
            ...
            sql = "select count(t.emp_id) from form_accessibility t where t.form_id=1890  and t.emp_id='" & user_id(0) & "' "
            ...
            End If
        End If
        Dim dts1 As DataTable = oh.ExecuteDataSet("select query from mactech.hrm_report_master where firm_id=99 and query_id=202").Tables(0)
        Dim strd() As String = dts1.Rows(0)(0).ToString.Split("#")
        'Dim dts1 As DataTable = oh.ExecuteDataSet("select t.empcode as EMPCODE, (select d.emp_name from mactech.employee_master d where d.emp_code = t.empcode) as EMPNAME, t.workeddate as WORKDATE, t.lopcancelltndate as CANCELDATE, (select d.emp_name from mactech.employee_master d where d.emp_code = t.recommender) as RECOMMENDER, (select d.emp_name from mactech.employee_master d where d.emp_code = t.approver) as APPROVER, upper(decode(t.status, 4, 'Recommended', 0, 'Applied', 1, 'Approved', 2, 'Rejected')) as STATUS, t.remarks as EMPLOYEE_REMARKS, t.recommenderremarks as RECOMMENDER_REMARKS, t.approverremarks as APPROVER_REMARKS from mactech.tbl_lop_cancelled t, mactech.employee_master mm where mm.emp_code = t.empcode and mm.firm_id = 8 and to_date(t.workeddate) between to_date('" & fdate & "') and to_date('" & tdate & "') order by t.workeddate").Tables(0)
        ...
        tt1.Text = "<b><font size=2>&nbsp;&nbsp;&nbsp;SUNDAY&nbsp;LOP&nbsp;CANCEL&nbsp;REPORT&nbsp;</font></b>"
        tt.Controls.Add(tt1)
        ...
        If ap = 1 Then
            ...
            strs = strd(0).Replace("empcd", sf(0))
            ' dt = oh.ExecuteDataSet(strs).Tables(0)

            stft = strs.Replace("fromdate", fdate)
            'dt = oh.ExecuteDataSet(stft).Tables(0)

            sttt = stft.Replace("todate", tdate)
            dt = oh.ExecuteDataSet(sttt).Tables(0)

        ElseIf ap = 2 Then
            sf = Session("user_id").ToString.Split("!")
            strs = strd(2).Replace("empcd", sf(0))
            ' dt = oh.ExecuteDataSet(strs).Tables(0)
        ...
        d0.ColumnSpan = 1
        d0.HorizontalAlign = HorizontalAlign.Right
