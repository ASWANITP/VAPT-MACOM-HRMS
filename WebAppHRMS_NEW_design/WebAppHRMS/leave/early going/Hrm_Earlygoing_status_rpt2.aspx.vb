...
Partial Class EXTRAFORMS_Hrm_Earlygoing_status_rpt1_87bf94913760
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        RH.DrawLine(tb, 44)
        '   dt = oh.ExecuteDataSet("select bm.BRANCH_NAME,e.emp_code,em.emp_name,d.dep_name,to_date(e.tra_dt),to_date(e.going_dt),e.reason,decode(e.status,1,'Sanction',2,'Reject',0,'Applied',4,'Recommended',5,'Cancelled',6,'Recommended') from hrm_earlygoing_appl e,employee_master em,branch_dtl_new bm,department_mst d where e.emp_code=em.emp_code and e.branch_id=bm.BRANCH_ID and d.dep_id=e.dep_id and to_date(e.going_dt)>=to_date('" & fromdate & "') and to_date(e.going_dt)<=to_date('" & todate & "') and em.status_id=1 order by e.tra_dt,bm.BRANCH_NAME").Tables(0)
        dt = oh.ExecuteDataSet("select bm.BRANCH_NAME,e.emp_code,em.emp_name,d.dep_name,to_date(e.tra_dt),to_date(e.going_dt),e.reason,decode(e.status,1,'Sanction',2,'Reject',0,'Applied',4,'Recommended',5,'Cancelled',6,'Recommended') from hrm_earlygoing_appl e,employee_master em,branch_dtl_new bm,department_mst d,employ_firm ef where e.emp_code=em.emp_code and e.branch_id=bm.BRANCH_ID and d.dep_id=e.dep_id and em.emp_code = ef.emp_code  and ef.firm_id = ' " & Session("firm_id") & " ' and to_date(e.going_dt)>=to_date('" & fromdate & "') and to_date(e.going_dt)<=to_date('" & todate & "') and em.status_id=1 order by e.tra_dt,bm.BRANCH_NAME").Tables(0)
