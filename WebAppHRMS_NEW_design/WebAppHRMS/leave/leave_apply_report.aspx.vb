...
Partial Class leave_leave_apply_report_2d673a029051
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            sql = "select em.emp_name,dm.designation||','||dem.dep_name,ed.perm_add1,pm.post_office||','||nvl(ed.cont_phone,0) from employee_master em,designation_master dm,department_mst dem,employ_personal_dtl ed,post_master pm where pm.sr_number=ed.perm_pin and ed.emp_code=em.emp_code and em.designation_id=dm.designation_id and em.department_id=dem.dep_id and em.emp_code=" & dt_emp.Rows(0)(0) & ""
            dt = oh.ExecuteDataSet(sql).Tables(0)
            sql = "select t.leave_id,t.leave_days from employ_leave_master t where t.emp_code=" & dt_emp.Rows(0)(0) & " order by leave_id"
            dt1 = oh.ExecuteDataSet(sql).Tables(0)
            sql = "select t.leave_id,decode(t.leave_id,1,sum(t.leave_days),2,sum(t.leave_days),3,sum(t.leave_days),sum(t.leave_days)) from employ_leave_dtl t where t.emp_code=" & dt_emp.Rows(0)(0) & " and t.leave_process_id not in (0,3) and to_char(leave_frdate,'YYYY')=to_char(sysdate,'YYYY') group by leave_id order by t.leave_id"
            ...
            Else
                If Session("branch_id") = 0 Then
                    ...
                    If dt4.Rows(0)(0) <> 173 Then
                        sql = "select dm.dep_name from employee_master t,department_mst dm where t.department_id=dm.dep_id and dm.dep_head is not null and t.emp_code=" & dt_emp.Rows(0)(0) & ""
                        dt4 = oh.ExecuteDataSet(sql).Tables(0)
                        If dt4.Rows.Count > 0 Then
                            ...
                            If dt4.Rows.Count > 0 Then
                                report.SetParameterValue("to_person", "CHAIRMAN")
                            Else
                                sql = "select nvl(pm.post_name||','||dm.dep_name,'0') from employee_master t,department_mst dm,post_mst pm,employee_master ed where t.department_id=dm.dep_id and dm.dep_head=ed.emp_code and ed.post_id=pm.post_id and t.emp_code=" & dt_emp.Rows(0)(0) & ""
                                dt4 = oh.ExecuteDataSet(sql).Tables(0)
                    ...
                    Else
                        ...
                        dt4 = oh.ExecuteDataSet(sql).Tables(0)
                        report.SetParameterValue("to_person", "ZONAL MANAGER, " & dt4.Rows(0)(0))
            ...
            report.SetParameterValue("code", dt_emp.Rows(0)(0))
