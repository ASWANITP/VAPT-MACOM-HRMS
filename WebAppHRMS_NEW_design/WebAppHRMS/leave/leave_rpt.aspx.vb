...
Partial Class leave_leave_rpt_1ebc980d5275
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Session("branch_id") = 0 Then
            ...
            Dim strp As String = "select el.emp_code from employee_master_dtl el where el.new_empcode=" & Me.Request.QueryString("emp_code")
            Dim pdt As DataTable = oh.ExecuteDataSet(strp).Tables(0)
            If pdt.Rows.Count = 0 Then
                Dim name As String = "select a.leave_frdate,a.leave_todate,a.leave_days,b.leave_type,a.leave_reason from employ_leave_dtl a,leave_master b where a.leave_process_id in (1,2) and a.leave_id=b.leave_id and a.emp_code=" & Request.QueryString.Get("emp_code") & " and to_date(leave_frdate)>='" & Request.QueryString.Get("fdt") & "' and to_date(leave_todate)<='" & Request.QueryString.Get("tdt") & "' order by to_date(leave_frdate)"
            Else
                Dim name As String = "select a.leave_frdate,a.leave_todate,a.leave_days,b.leave_type,a.leave_reason from employ_leave_dtl a,leave_master b where a.leave_process_id in (1,2) and a.leave_id=b.leave_id and (a.emp_code=" & Request.QueryString.Get("emp_code") & " or a.emp_code=" & pdt.Rows(0)(0) & " )  and to_date(leave_frdate)>='" & Request.QueryString.Get("fdt") & "' and to_date(leave_todate)<='" & Request.QueryString.Get("tdt") & "' order by to_date(leave_frdate)"
            End If
        ...
        Else
            ...
            Dim strp As String = "select el.emp_code from employee_master_dtl el where el.new_empcode=" & ar(0)
            Dim pdt As DataTable = oh.ExecuteDataSet(strp).Tables(0)
            If pdt.Rows.Count = 0 Then
                Dim name As String = "select a.leave_frdate,a.leave_todate,a.leave_days,b.leave_type,a.leave_reason from employ_leave_dtl a,leave_master b where a.leave_process_id in (1,2) and a.leave_id=b.leave_id and a.emp_code=" & ar(0) & " and to_date(leave_frdate)>='" & Request.QueryString.Get("fdt") & "' and to_date(leave_todate)<='" & Request.QueryString.Get("tdt") & "' order by to_date(leave_frdate)"
            Else
                Dim name As String = "select a.leave_frdate,a.leave_todate,a.leave_days,b.leave_type,a.leave_reason from employ_leave_dtl a,leave_master b where a.leave_process_id in (1,2) and a.leave_id=b.leave_id and (a.emp_code=" & ar(0) & " or a.emp_code=" & pdt.Rows(0)(0) & " ) and to_date(leave_frdate)>='" & Request.QueryString.Get("fdt") & "' and to_date(leave_todate)<='" & Request.QueryString.Get("tdt") & "' order by to_date(leave_frdate)"
            ...
            dt1 = oh.ExecuteDataSet("select a.emp_code,a.emp_name,case when a.emp_code in(select em.new_empcode from employee_master_dtl em ) then (select join_dt from employee_master e1 ,employee_master_dtl em1 where em1.new_empcode=" & ar(0) & " and em1.emp_code=e1.emp_code ) else a.join_dt end  as join_dt,b.designation, c.dep_name, d.branch_name from employee_master    a, designation_master b, department_mst     c, branch_master      d where a.designation_id = b.designation_id and a.department_id = c.dep_id and a.branch_id = d.branch_id and a.emp_code = " & ar(0)).Tables(0)
