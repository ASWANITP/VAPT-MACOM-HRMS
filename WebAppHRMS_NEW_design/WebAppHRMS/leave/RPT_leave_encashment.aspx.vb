...
Partial Class ENCASHMENT_RPT_leave_encashment_c0dc5f405333
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        RH.Heading(Session("branch_id"), Session("branch_name"), Session("firm_name"), tb, "EARNED LEAVE ENCASHMENT REQUESTED LIST", 32)
        ...
        RH.AddColumn(tr07, tr07_07, 2, 10, "c", "&nbsp;&nbsp;CLAIMED&nbsp;")
        RH.AddColumn(tr07, tr07_08, 2, 10, "c", "&nbsp;&nbsp;BALANCE&nbsp;&nbsp;")
        tb.Controls.Add(tr07)
        RH.DrawLine(tb, 32)
        ' dt = oh.ExecuteDataSet("select a.emp_id,b.emp_name,c.branch_name,d.post_name,a.earned_leave,a.encash_leave,(a.earned_leave-a.encash_leave) as balance from hrm_earned_leave a,employee_master b,branch_master c,post_mst d where a.emp_id = b.emp_code And b.branch_id = c.branch_id And b.post_id = d.post_id and a.earned_leave>5 and a.encash_leave<>0 and b.status_id in (1,10) order by b.emp_code").Tables(0)
        dt = oh.ExecuteDataSet("select a.emp_id,  b.emp_name,  c.branch_name,  case  when b.post_id in (select post_id from post_mst) then  (select post_name from post_mst p where p.post_id = b.post_id)  else  'NIL'  end as post,  a.earned_leave,  a.encash_leave,  (a.earned_leave - a.encash_leave) as balance  from hrm_earned_leave a, employee_master b, branch c,employ_firm ef  where a.emp_id = b.emp_code   And b.branch_id = c.branch_id  and a.earned_leave > 5  and a.salary is not null  and b.emp_code=ef.emp_code  and ef.firm_id='" & Session("firm_id") & "'  and b.status_id in (1, 10)  order by b.emp_code").Tables(0)
        ...
        For Each dr In dt.Rows
            ...
            RH.AddColumn(tr09, tr09_08, 2, 10, "c", dr(6))
