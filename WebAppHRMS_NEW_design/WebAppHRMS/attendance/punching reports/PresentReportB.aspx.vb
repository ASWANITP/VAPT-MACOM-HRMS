...
Partial Class Attendence_Report_PresentReportB_0e8de44c1082
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        sql = "select branch_id,branch_name, count(emp_code) as Total, sum(case when (m_time is NOT NULL or e_time is NOT NULL) then 1 else 0 end) as present,sum(case  when (m_time is NULL and e_time is NULL) then  1 else 0 end) as Absent, sum(case  when (m_time > in_time) then 1 else 0 end) as Late, sum(case when (e_time < out_time) then  1 else  0 end) as EarlyGoing, sum(case  when ((m_time is null and e_time is not null) OR (M_TIME IS not NULL AND E_TIME IS NULL and CURR_DATE < to_date(sysdate))) then 1 else 0 end) as NonMarking,nvl(other_total,0),nvl(other_present,0),nvl(other_absent,0),other_late,other_earlygoing,other_nonmarking from (select * from attendance_detail_1 da   where da.CURR_DATE = to_date('" & frdate & "') and area_id=" & areaid & "   and department_id = 14  and post_id not in (173, 141, 136, 197, 28, 200, 195, 199) and shift_id not in (4, 5))  left outer join (select branch_id,branch_name,count(emp_code) as Total, sum(case  when (m_time is NOT NULL or e_time is NOT NULL) then 1  else   0 end) as present, sum(case when (m_time is NULL and e_time is NULL) then  1  else 0 end) as Absent, sum(case when (m_time > in_time) then  1 else 0 end) as Late, sum(case when (e_time < out_time) then 1 else 0  end) as EarlyGoing,  sum(case  when ((m_time is null and e_time is not null) OR (M_TIME IS not NULL AND E_TIME IS NULL and CURR_DATE < to_date(sysdate))) then  1  else 0  end) as NonMarking from attendance_detail_1  where curr_date = to_date('" & frdate & "') and area_id=" & areaid & "   and department_id = 14 and post_id not in (173, 141, 136, 197, 28, 200, 195, 199) and shift_id not in (4, 5) group by branch_id,branch_name) using (branch_id,branch_name) left outer join (select branch_id,branch_name,count(emp_code) as other_Total,sum(case when (m_time is NOT NULL or e_time is NOT NULL) then 1 else  0 end) as other_present, sum(case when (m_time is NULL and e_time is NULL) then 1 else 0 end) as other_Absent, sum(case when (m_time > in_time) then  1 else 0 end) as other_Late, sum(case when (e_time < out_time) then  1 else   0 end) as other_EarlyGoing, sum(case when ((m_time is null and e_time is not null) OR (M_TIME IS not NULL AND E_TIME IS NULL and  CURR_DATE < to_date(sysdate))) then 1 else 0 end) as other_NonMarking from attendance_detail_1 where curr_date = to_date('" & frdate & "') and area_id=" & areaid & "  and department_id = 14 and post_id  in (173, 141, 136, 197, 28, 200, 195, 199) and shift_id not in (4, 5) group by branch_id,branch_name)  using  (branch_id,branch_name) group by branch_id,branch_name,other_total,other_present,other_absent,other_late,other_earlygoing,other_nonmarking"
