...
Partial Class Attendence_Report_PresentReportD_31d5029f6985
    Inherits System.Web.UI.Page
    Dim dt, dt1, dtr As New DataTable
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        ' sql = "select count(da.emp_code),DM.DIV_NAME,DM.DIVISION_ID,sum(case when (da.m_time is NOT NULL or da.e_time is NOT NULL) then 1 else 0 end) as present,sum(case when (da.m_time is NULL and da.e_time is NULL) then 1 else 0 end) as Absent,sum(case when (da.shift_id=tt.shift_id and da.m_time>tt.in_time) then 1 else 0 end) as Late,sum(case when (da.shift_id=tt.shift_id and da.e_time<tt.out_time) then 1 else 0 end) as EarlyGoing,sum(case when ((da.m_time is  null or da.e_time is  null) AND NOT(DA.M_TIME IS NULL AND DA.E_TIME IS NULL)) then 1 else 0 end) as NonMarking from time_tab tt, ATTENDANCE da,branch_master bm,AREA_DETAIL AD,DIVISION_DETAIL DD,DIVISION_MASTER DM,REGION_DETAIL RD where tt.shift_id=da.shift_id and bm.branch_id=da.branch_id  and da.branch_id=AD.BRANCH_ID AND AD.AREA_ID=DD.AREA_ID AND DD.DIV_ID=DM.DIVISION_ID AND DM.DIVISION_ID=RD.DIVISION_ID AND RD.REGION_ID=" & regid & " and da.curr_date>=to_date('" & frdate & "') and da.curr_date<=to_date('" & frdate & "') group by DM.DIV_NAME,DM.DIVISION_ID"
        sql = "select div_id,div_name,count(emp_code) as Total, sum(case when (m_time is NOT NULL or e_time is NOT NULL) then 1 else 0 end) as present,sum(case  when (m_time is NULL and e_time is NULL) then  1 else 0 end) as Absent, sum(case  when (m_time > in_time) then 1 else 0 end) as Late, sum(case when (e_time < out_time) then  1 else  0 end) as EarlyGoing, sum(case  when ((m_time is null and e_time is not null) OR (M_TIME IS not NULL AND E_TIME IS NULL and CURR_DATE < to_date(sysdate))) then 1 else 0 end) as nonMarking,nvl(other_total,0),nvl(other_present,0),nvl(other_absent,0),other_late,other_earlygoing,other_nonmarking from (select * from attendance_detail_1 da   where da.CURR_DATE = to_date('" & frdate & "') and da.region_id =" & regid & "  and department_id = 14  and post_id not in (173, 141, 136, 197, 28, 200, 195, 199) and shift_id not in (4, 5))  left outer join (select div_id,div_name,count(emp_code) as Total, sum(case  when (m_time is NOT NULL or e_time is NOT NULL) then 1  else   0 end) as present, sum(case when (m_time is NULL and e_time is NULL) then  1  else 0 end) as Absent, sum(case when (m_time > in_time) then  1 else 0 end) as Late, sum(case when (e_time < out_time) then 1 else 0  end) as EarlyGoing,  sum(case  when ((m_time is null and e_time is not null) OR (M_TIME IS not NULL AND E_TIME IS NULL and CURR_DATE < to_date(sysdate))) then  1  else 0  end) as NonMarking from attendance_detail_1  where curr_date = to_date('" & frdate & "') and region_id =" & regid & "  and department_id = 14 and post_id not in (173, 141, 136, 197, 28, 200, 195, 199) and shift_id not in (4, 5) group by div_id,div_name) using (div_id,div_name) left outer join (select div_id,div_name,count(emp_code) as other_Total,sum(case when (m_time is NOT NULL or e_time is NOT NULL) then 1 else  0 end) as other_present, sum(case when (m_time is NULL and e_time is NULL) then 1 else 0 end) as other_Absent, sum(case when (m_time > in_time) then  1 else 0 end) as other_Late, sum(case when (e_time < out_time) then  1 else   0 end) as other_EarlyGoing, sum(case when ((m_time is null and e_time is not null) OR (M_TIME IS not NULL AND E_TIME IS NULL and CURR_DATE < to_date(sysdate))) then 1 else 0 end) as other_NonMarking from attendance_detail_1 where curr_date = to_date('" & frdate & "') and region_id =" & regid & " and department_id = 14 and post_id  in (173, 141, 136, 197, 28, 200, 195, 199) and shift_id not in (4, 5) group by div_id,div_name)  using  (div_id,div_name) group by div_id,div_name,other_total,other_present,other_absent,other_late,other_earlygoing,other_nonmarking"
        ...
        sql = "select initcap(r.reg_name) from region_master r where r.reg_id=" & regid
        dtr = oh.ExecuteDataSet(sql).Tables(0)
        ...
        td41.Text = "<font size=3><b> Attendance Report of &nbsp " & dtr.Rows(0)(0) & "&nbspRegion&nbspFrom :&nbsp" & frdate & " &nbsp To :" & frdate & " </b></font>"
