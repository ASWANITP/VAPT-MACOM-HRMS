...
Partial Class Attendence_Report_PresentReportA_08a60f518794
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        sql = "select area_id,  area_name,  count(emp_code) as Total,  sum(case  when (m_time is NOT NULL or e_time is NOT NULL) then  1  else  0  end) as present,  sum(case  when (m_time is NULL and e_time is NULL) then  1  else  0  end) as Absent,  sum(case  when (m_time > in_time) then  1  else  0  end) as Late,  sum(case  when (e_time < out_time) then  1  else  0  end) as EarlyGoing,  sum(case  when ((m_time is null and e_time is not null) or  (M_TIME IS not NULL AND E_TIME IS NULL and  curr_date < to_date(sysdate))) then  1  else  0  end) as NonMarking  from (select da.*  from attendance_detail da,branch_master b  where da.CURR_DATE >= to_date('" & frdate & "')  and da.curr_date <= to_date('" & frdate & "')  and da.div_id = " & divid & "  and da.BRANCH_ID=b.branch_id  and b.firm_id=" & Session("firm_id") & "  and da.shift_id not in (4, 5))  left outer join (select a.area_id,  a.area_name,  count(a.emp_code) as Total,  sum(case  when (a.m_time is NOT NULL or a.e_time is NOT NULL) then  1  else  0  end) as present,  sum(case  when (a.m_time is NULL and a.e_time is NULL) then  1  else  0  end) as Absent,  sum(case  when (a.m_time > a.in_time) then  1  else  0  end) as Late,  sum(case  when (a.e_time < a.out_time) then  1  else  0  end) as EarlyGoing,  sum(case  when ((a.m_time is null and a.e_time is not null) or  (a.M_TIME IS not NULL AND a.E_TIME IS NULL and  a.curr_date < to_date(sysdate))) then  1  else  0  end) as NonMarking  from attendance_detail a,branch_master b  where a.CURR_DATE >= to_date('" & frdate & "')  and a.curr_date <= to_date('" & frdate & "')  and a.div_id = " & divid & "  and a.BRANCH_ID=b.branch_id  and b.firm_id=" & Session("firm_id") & "  and a.shift_id not in (4, 5)  group by a.area_id, a.area_name)  using (area_id, area_name)  group by area_id, area_name"
