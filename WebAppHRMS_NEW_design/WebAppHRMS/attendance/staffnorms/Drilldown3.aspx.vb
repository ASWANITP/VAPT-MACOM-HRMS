...
Partial Class Drilldown3_06e615717173
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Dim dt2 As DataTable = oh.ExecuteDataSet("select div_name from division_master where division_id=" & Request.QueryString("division_id") & "").Tables(0)
        ...
        tdr2.Text = "<font size=3 color=red><b>DIVISION-&nbsp;" & dt2.Rows(0)(0) & "  </b></font>"
        ...
        ta52.Text = "<font size=2><b>AREA</b></font>"
        ...
        str = "select am.area_id,count(sr.branch_id),am.area_name,round(sum(sr.bh)+sum(sr.abh)+sum(sr.jr_asst)+sum(sr.sweeper)+sum(sr.jo))as as_per_norms,round(sum(sr.bh_avbl)+sum(sr.abh_avbl)+sum(sr.jr_asst_avbl)+sum(sr.sweeper_avbl)+sum(sr.jo_avbl)) as actual,round(sum(sr.bh_avbl)+sum(sr.abh_avbl)+sum(sr.jr_asst_avbl)+sum(sr.jo_avbl)) as others,sum(sr.sweeper_avbl) as sweeper,nvl(sum(case when sr.jr_asst-sr.jr_asst_avbl<0 then 0 else sr.jr_asst-sr.jr_asst_avbl end),0) as short_jr,nvl(sum(case when sr.sweeper-sr.sweeper_avbl<0 then 0 else sr.sweeper-sr.sweeper_avbl end),0) as short_sweeper,nvl(sum(case when sr.jr_asst_avbl-sr.jr_asst<0 then 0 else sr.jr_asst_avbl-sr.jr_asst end),0) as surplus,sum(sr.long_leave) as long_leave,sum(sr.fldstaff_gold) as gl_rc_norm,sum(sr.fldstaff_gold_avbl) as gl_rc_actual,nvl(sum(case when sr.fldstaff_gold-sr.fldstaff_gold_avbl<0 then 0 else sr.fldstaff_gold-sr.fldstaff_gold_avbl end),0) as short_gl_rc,nvl(sum(case when sr.fldstaff_gold_avbl-sr.fldstaff_gold<0 then 0 else sr.fldstaff_gold_avbl-sr.fldstaff_gold end),0) as surplus_gl_rc,sum(sr.gl_marketing) as gl_mkt_actual,nvl(sum(case when sr.jo-sr.jo_avbl<0 then 0 else sr.jo-sr.jo_avbl end),0) as short_jofficer,nvl(sum(case when sr.jo_avbl-sr.jo<0 then 0 else sr.jo_avbl-sr.jo end),0) as surplus_JO from staff_required sr,area_detail ad,area_master am,division_detail dd,region_detail rd,zonal_detail zd where sr.branch_id=ad.branch_id and ad.area_id=dd.area_id and dd.div_id=rd.division_id and ad.area_id=am.area_id and dd.div_id=" & Request.QueryString("division_id") & " and rd.region_id=zd.region_id  and rd.region_id=" & Request.QueryString("region_id") & " and zd.zonal_id=" & Request.QueryString("zone_id") & " and sr.branch_id<>0 group by am.area_id,am.area_name"
        ...
        For Each dr In dt.Rows
            ...
            lm52.Text = "<a href=Drilldown4.aspx?zone_id=" & Request.QueryString("zone_id") & "&region_id=" & Request.QueryString("region_id") & "&division_id=" & Request.QueryString("division_id") & "&area_id=" & dr(0) & "><font size=2> " & dr(2) & "&nbsp;</font>"
        ...
        lin212.Text = "<a href=Drilldown2.aspx?zone_id=" & Request.QueryString("zone_id") & "&region_id=" & Request.QueryString("region_id") & "><font color=blue>BACK</font ></a>"
