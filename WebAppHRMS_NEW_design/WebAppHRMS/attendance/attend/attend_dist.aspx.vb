...
Partial Class attend_attend_dv_f71f82185031
    Inherits System.Web.UI.Page

    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Dim dt As New DataTable
        Dim arun As String = "select dm.district_name,count(distinct t.emp_code),dm.district_id from attend t,branch_master bm,state_master st,district_master dm where  bm.district_id=dm.district_id and st.state_id=bm.state_id and st.state_id=" & Request.QueryString("id") & " and t.branch_id=bm.branch_id and emp_code<9999 and shift_id in(" & Request.QueryString("shift") & ") and to_date(curr_date)>='" & Request.QueryString("fdt") & "' and to_date(curr_date)<='" & Request.QueryString("tdt") & "' and (not t.m_time is null and not t.e_time is null)and shift_id in (" & Request.QueryString("shift") & ")  group by dm.district_id,dm.district_name union all select dm.district_name,count(distinct t.emp_code),dm.district_id  from district_master dm,daily_attend t,branch_master bm,state_master st where bm.district_id=dm.district_id and st.state_id=" & Request.QueryString("id") & " and  st.state_id=bm.state_id and t.branch_id=bm.branch_id and emp_code<9999 and shift_id in(" & Request.QueryString("shift") & ") and to_date(curr_date)>='" & Request.QueryString("fdt") & "' and to_date(curr_date)<='" & Request.QueryString("tdt") & "' and ( not t.m_time is null and not t.e_time is null)and shift_id in (" & Request.QueryString("shift") & ") group by dm.district_id,dm.district_name"
        ...
        tct3.ColumnSpan = 2
        tct3.Text = "<b><font size=2 > Attendence Report Punched Districts </font></b>"
        ...
        For Each ar In dt.Rows
            ...
            'Dim p As Integer = -1
            t1.Text = "<a href=attend_br.aspx?did=" & ar(2) & "&shift=" & Request.QueryString("shift") & "&fdt=" & Request.QueryString("fdt") & "&tdt=" & Request.QueryString("tdt") & "&stid=" & Request.QueryString("id") & ">" & ar(0) & "</a>"
            t.Cells.Add(t1)
            t2.Text = ar(1)
            c = c + ar(1)
