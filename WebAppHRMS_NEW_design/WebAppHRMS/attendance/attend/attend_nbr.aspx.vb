...
Partial Class attend_attend_br_f1ce36f01735
    Inherits System.Web.UI.Page

    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Dim dt As New DataTable
        Dim arun As String = "select bm.branch_name,count(distinct t.emp_code),bm.branch_id from attend t,branch_master bm,state_master st,division_detail dt,division_master dm,area_detail ad  where bm.branch_id=ad.branch_id and ad.area_id=dt.area_id and dt.div_id=dm.division_id and dt.div_id=" & Request.QueryString.Get("did") & " and st.state_id=bm.state_id and st.state_id=" & Request.QueryString.Get("stid") & " and t.branch_id=bm.branch_id  and emp_code<9999 and shift_id in(" & Request.QueryString("shift") & ") and to_date(curr_date)>='" & Request.QueryString("fdt") & "' and to_date(curr_date)<='" & Request.QueryString("tdt") & "' and ( t.m_time is null or  t.e_time is null)and shift_id in (" & Request.QueryString("shift") & ")  group by bm.branch_name,bm.branch_id union all select bm.branch_name,count(distinct t.emp_code),bm.branch_id  from division_master dm,division_detail dt,daily_attend t,branch_master bm,state_master st,area_detail ad  where bm.branch_id=ad.branch_id and ad.area_id=dt.area_id and  dm.division_id=dt.div_id and dt.div_id=" & Request.QueryString("did") & " and st.state_id=" & Request.QueryString("stid") & " and  st.state_id=bm.state_id and t.branch_id=bm.branch_id and emp_code<9999 and shift_id in(" & Request.QueryString("shift") & ") and to_date(curr_date)>='" & Request.QueryString("fdt") & "' and to_date(curr_date)<='" & Request.QueryString("tdt") & "' and ( t.m_time is null or  t.e_time is null)and shift_id in (" & Request.QueryString("shift") & ") group by bm.branch_name,bm.branch_id"
        ...
        tct3.ColumnSpan = 2
        tct3.Text = "<b><font size=2 > Attendence Not Punched Branches  Of Division " & ss & "</font></b>"
        ...
        For Each ar In dt.Rows
            ...
            'Dim p As Integer = -1
            t1.Text = "<a href=attend_nemp.aspx?id=" & ar(2) & "&shift=" & Request.QueryString("shift") & "&fdate=" & Request.QueryString("fdt") & "&tdate=" & Request.QueryString("tdt") & ">" & ar(0) & "</a>"
            t.Cells.Add(t1)
            t2.Text = ar(1)
            c = c + ar(1)
