...
Partial Class attend_attend_br_f1ce36f03121
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Dim arun As String
        If dat = 0 Then
            '            arun = "select bm.branch_name,count(distinct t.emp_code) as count,bm.branch_id from daily_attend t,branch_master bm,state_master st,division_detail dt,division_master dm where bm.branch_id=dt.branch_id and dt.division_id=dm.division_id and dt.division_id=" & Request.QueryString.Get("did") & " and st.state_id=bm.state_id and st.state_id=" & Request.QueryString.Get("stid") & " and t.branch_id=bm.branch_id and dt.branch_id=bm.branch_id and emp_code<9999 and shift_id in(" & Request.QueryString("shift") & ") and to_date(curr_date)='" & Request.QueryString("fdt") & "'  and (not t.m_time is null or not t.e_time is null) and shift_id in (" & Request.QueryString("shift") & ")  group by bm.branch_name,bm.branch_id "
            arun = "select bm.branch_name,count(distinct t.emp_code) as count,bm.branch_id from daily_attend t,branch_master bm,state_master st,division_detail dt,division_master dm,area_detail ad where bm.branch_id=ad.branch_id and ad.area_id=dt.area_id and dt.div_id=dm.division_id and dt.div_id=" & Request.QueryString.Get("did") & " and st.state_id=bm.state_id and st.state_id=" & Request.QueryString.Get("stid") & " and t.branch_id=bm.branch_id  and emp_code<9999 and shift_id in(" & Request.QueryString("shift") & ") and to_date(curr_date)='" & Request.QueryString("fdt") & "' and (not t.m_time is null or not t.e_time is null) and shift_id in (" & Request.QueryString("shift") & ")  group by bm.branch_name,bm.branch_id "
        Else
            arun = "select bm.branch_name,count(distinct t.emp_code) as count,bm.branch_id from attend t,branch_master bm,state_master st,division_detail dt,division_master dm,area_detail ad where bm.branch_id=ad.branch_id and ad.area_id=dt.area_id and dt.div_id=dm.division_id and dt.div_id=" & Request.QueryString.Get("did") & " and st.state_id=bm.state_id and st.state_id=" & Request.QueryString.Get("stid") & " and t.branch_id=bm.branch_id  and emp_code<9999 and shift_id in(" & Request.QueryString("shift") & ") and to_date(curr_date)='" & Request.QueryString("fdt") & "' and (not t.m_time is null or not t.e_time is null) and shift_id in (" & Request.QueryString("shift") & ")  group by bm.branch_name,bm.branch_id "
        End If
        ...
        Dim ss As String = oh.ExecuteDataSet("select div_name from division_master where division_id=" & Request.QueryString("did")).Tables(0).Rows(0)(0)

        Dim tct3 As New TableCell
        tct3.ColumnSpan = 2
        tct3.Text = "<b><font size=2 > Attendence Report Punched Branches Of Division " & ss & " </font></b>"
        ...
        For Each ar In dt.Rows
            ...
            'Dim sql As String = "select bm.branch_name,count(distinct t.emp_code) as sum,bm.branch_id  from division_master dm,division_detail dt,daily_attend t,branch_master bm,state_master st where dm.division_id=dt.division_id and dt.division_id=" & Request.QueryString("did") & " and st.state_id=" & Request.QueryString("stid") & " and dt.branch_id=bm.branch_id and dt.branch_id=bm.branch_id and st.state_id=bm.state_id and t.branch_id=bm.branch_id and t.branch_id=" & ar(2) & " and emp_code<9999 and shift_id in(" & Request.QueryString("shift") & ") and to_date(curr_date)>='" & Request.QueryString("fdt") & "' and to_date(curr_date)<='" & Request.QueryString("tdt") & "' and (not t.m_time is null or not t.e_time is null)and shift_id in (" & Request.QueryString("shift") & ") group by bm.branch_name,bm.branch_id"
            ...
            'Dim p As Integer = -1
            t1.Text = "<a href=attend_emp.aspx?id=" & ar(2) & "&shift=" & Request.QueryString("shift") & "&fdate=" & Request.QueryString("fdt") & " > " & ar(0) & "</a>"
            t.Cells.Add(t1)
            t2.Text = ar(1)
