...
Partial Class Attendence_Report_PresentReportD_31d5029f8983
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        sql = "select div_id,  div_name,  count(emp_code) as Total,  sum(case  when (m_time is NOT NULL or e_time is NOT NULL) then  1  else  0  end) as present,  sum(case  when (m_time is NULL and e_time is NULL) then  1  else  0  end) as Absent,  sum(case  when (m_time > in_time) then  1  else  0  end) as Late,  sum(case  when (e_time < out_time) then  1  else  0  end) as EarlyGoing,  sum(case  when ((m_time is null and e_time is not null) OR  (M_TIME IS not NULL AND E_TIME IS NULL and  CURR_DATE < to_date(sysdate))) then  1  else  0  end) as NonMarking  from (select da.*  from attendance_detail da,branch_master b  where da.CURR_DATE >= to_date('" & frdate & "')  and da.curr_date <= to_date('" & frdate & "')  and da.region_id = " & regid & "  and da.BRANCH_ID=b.branch_id  and b.firm_id=" & Session("firm_id") & "  and da.shift_id not in (4, 5))  left outer join (select a.div_id,  a.div_name,  count(a.emp_code) as Total,  sum(case  when (a.m_time is NOT NULL or a.e_time is NOT NULL) then  1  else  0  end) as present,  sum(case  when (a.m_time is NULL and a.e_time is NULL) then  1  else  0  end) as Absent,  sum(case  when (a.m_time > a.in_time) then  1  else  0  end) as Late,  sum(case when (a.e_time < out_time) then  1  else  0  end) as EarlyGoing,  sum(case  when ((a.m_time is null and a.e_time is not null) or  (a.M_TIME IS not NULL AND a.E_TIME IS NULL and  a.CURR_DATE < to_date(sysdate))) then  1  else  0  end) as NonMarking  from attendance_detail a,branch_master b  where a.CURR_DATE >= to_date('" & frdate & "')  and a.curr_date <= to_date('" & frdate & "')  and a.region_id = " & regid & "  and a.shift_id not in (4, 5)  and a.BRANCH_ID=b.branch_id  and b.firm_id=" & Session("firm_id") & "  group by a.div_id, a.div_name)  using (div_id, div_name)  group by div_id, div_name"
