...
Partial Class Attendence_Report_PresentReport_08710abd9928
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Select Case category
            Case 1
                sql = "select count(da.emp_code), rm.reg_name, rm.reg_id  from ATTENDANCE      da,  branch_master   bm,  AREA_DETAIL     AD,  DIVISION_DETAIL DD,  REGION_DETAIL   RD,  region_master   rm  where bm.branch_id = da.branch_id  and da.branch_id = AD.BRANCH_ID  AND AD.AREA_ID = DD.AREA_ID  AND DD.DIV_ID = RD.DIVISION_ID  AND RD.REGION_ID = rm.reg_id  and da.curr_date >= to_date('" & frdate & "')  and da.curr_date <= to_date('" & todate & "')  and (da.m_time is NOT NULL or da.e_time is NOT NULL or  da.pay_id in (50, 51, 52))  and bm.firm_id = " & Session("firm_id") & "  group by rm.reg_name, rm.reg_id"

                cat = "PRESENT"
            Case 2
                sql = "select count(da.emp_code),rm.reg_name,rm.reg_id from ATTENDANCE da,branch_master bm,AREA_DETAIL AD,DIVISION_DETAIL DD,REGION_DETAIL RD,region_master rm where bm.branch_id=da.branch_id  and da.branch_id=AD.BRANCH_ID AND AD.AREA_ID=DD.AREA_ID AND DD.DIV_ID=RD.DIVISION_ID AND RD.REGION_ID=rm.reg_id and da.curr_date>=to_date('" & frdate & "') and da.curr_date<=to_date('" & todate & "') and da.m_time is NULL and da.e_time is NULL and da.pay_id not in (50,51,52) and bm.firm_id = " & Session("firm_id") & "  group by rm.reg_name,rm.reg_id"
                cat = "ABSENT"
            Case 3
                sql = "select count(da.emp_code),rm.reg_name,rm.reg_id from ATTENDANCE da,time_tab tt,branch_master bm,AREA_DETAIL AD,DIVISION_DETAIL DD,REGION_DETAIL RD,region_master rm where bm.branch_id=da.branch_id  and da.branch_id=AD.BRANCH_ID AND AD.AREA_ID=DD.AREA_ID AND DD.DIV_ID=RD.DIVISION_ID AND RD.REGION_ID=rm.reg_id and da.curr_date>=to_date('" & frdate & "') and da.curr_date<=to_date('" & todate & "') and da.m_shift=tt.shift_id and da.m_time>tt.in_time and da.pay_id not in (50,52,7) and bm.firm_id = " & Session("firm_id") & " group by rm.reg_name,rm.reg_id"
                cat = "LATE"
            Case 4
                sql = "select count(da.emp_code),rm.reg_name,rm.reg_id from ATTENDANCE da,time_tab tt,branch_master bm,AREA_DETAIL AD,DIVISION_DETAIL DD,REGION_DETAIL RD,region_master rm where bm.branch_id=da.branch_id  and da.branch_id=AD.BRANCH_ID AND AD.AREA_ID=DD.AREA_ID AND DD.DIV_ID=RD.DIVISION_ID AND RD.REGION_ID=rm.reg_id and da.curr_date>=to_date('" & frdate & "') and da.curr_date<=to_date('" & todate & "') and da.e_shift=tt.shift_id and da.e_time<tt.out_time  and da.pay_id not in (51,52,7) and bm.firm_id = " & Session("firm_id") & " group by rm.reg_name,rm.reg_id"
                cat = "EARLY GOING"
            Case 5
                sql = "select count(da.emp_code),rm.reg_name,rm.reg_id from ATTENDANCE da,branch_master bm,AREA_DETAIL AD,DIVISION_DETAIL DD,REGION_DETAIL RD,region_master rm where bm.branch_id=da.branch_id  and da.branch_id=AD.BRANCH_ID AND AD.AREA_ID=DD.AREA_ID AND DD.DIV_ID=RD.DIVISION_ID AND RD.REGION_ID=rm.reg_id and da.curr_date>=to_date('" & frdate & "') and da.curr_date<=to_date('" & todate & "') and ((da.m_time is  null or da.e_time is  null) AND NOT(DA.M_TIME IS NULL AND DA.E_TIME IS NULL)) and bm.firm_id = " & Session("firm_id") & "  group by rm.reg_name,rm.reg_id"
                cat = "NON MARKING"
        End Select
        ...
        td41.Text = "<font size=3><hr size='4' NOSHADE></font>"
        td41.Text = "<font size=3><b>" & cat & " &nbsp Report From :&nbsp" & frdate & " &nbsp To :" & todate & " </b></font>"
        ...
        For Each dr In dt.Rows
            ...
            td61.Text = "<font size=2.5><a href = javascript:next(" & dr(2) & ",'" & frdate & "','" & todate & "'," & category & ")>" & dr(1) & "</a></font>"
            ' 
            ...
            ' sql1 = "select count(em.emp_code) from employee_master em,branch_master bm,AREA_DETAIL AD,DIVISION_DETAIL DD,REGION_DETAIL RD where bm.branch_id=em.branch_id  and em.branch_id=AD.BRANCH_ID AND AD.AREA_ID=DD.AREA_ID AND DD.DIV_ID=RD.DIVISION_ID AND RD.REGION_ID=" & dr(2) & " and em.status_id=1"
            sql1 = "select count(da.emp_code),rd.region_id from ATTENDANCE da,branch_master bm,AREA_DETAIL AD,DIVISION_DETAIL DD,REGION_DETAIL RD where bm.branch_id=da.branch_id  and da.branch_id=AD.BRANCH_ID AND AD.AREA_ID=DD.AREA_ID AND DD.DIV_ID=RD.DIVISION_ID AND RD.REGION_ID=" & dr(2) & " and da.curr_date>=to_date('" & frdate & "') and da.curr_date<=to_date('" & todate & "')  group by rd.region_id"
            dt1 = oh.ExecuteDataSet(sql1).Tables(0)
            If (dr(0) <> 0) Or dt1.Rows(0)(0) = 0 Then
