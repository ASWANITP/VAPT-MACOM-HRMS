...
Partial Class specificempattend_individualreportm_1e447a931491
    Inherits System.Web.UI.Page
    Dim dt, dt1, dt6, dt7, dt8, dt9, dt10, dt11, dt12, dt13 As New DataTable
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        'dt13 = oh.ExecuteDataSet("select department_id from employee_master where emp_code=" & empcode & "").Tables(0)
        'Dim dts1 As DataTable = oh.ExecuteDataSet("select query from hrm_report_master where firm_id=99 and query_id=215").Tables(0)
        'Dim strd() As String = dts1.Rows(0)(0).ToString.Split("#")
        If (firm = 8) Then
            'dt = oh.ExecuteDataSet(strd).Tables(0)

            'sql = "select curr_date as day,decode(da.m_time,NULL,'----------',da.m_time) as morning_time,decode(e_time,NULL,'----------',e_time) as evening_time, case when (da.m_time is null and da.pay_id not in (50,52)) and (da.pay_id not in (51,52) and da.e_time is null) then 'Absent' else case when da.pay_id in (50) and da.e_time is not null then 'Morning-REG' else case when da.pay_id in (51) and da.m_time is not null then 'Evening-REG' else case when da.pay_id in (52) then 'BOTH-REG' else case when (da.m_time>bt1.in_time and da.m_time<>'TOUR' and da.m_time<>'COMPEN' and da.pay_id not in (50,7,52)) and (da.e_time is null and da.pay_id not in (51,7,52)) then 'Late & Non-Marking' else case when da.m_time<=bt1.in_time and (da.e_time is null and da.pay_id not in (51,52)) then case when da.PAY_ID in(7) then 'Absent' else 'Non-Marking Evening' end else case when (da.m_time is null and da.pay_id not in (50,52,7)) and da.e_time <bt2.out_time then 'Non-Marking Morning & Early-Going' else case when (da.m_time is null and da.pay_id not in (50,52)) and da.e_time >=bt2.out_time then case when da.PAY_ID in(7) then 'Absent' else  'Non-Marking Morning' end else case when da.m_time<=bt1.in_time and (da.e_time<bt2.out_time and da.e_time<>'TOUR' and da.e_time<>'COMPEN' and da.pay_id not in (51,52,7)) then 'Early-Going' else case when (da.m_time>bt1.in_time and da.pay_id not in (50,52) ) and (da.e_time<bt2.out_time and da.pay_id not in (51,52,7)) then 'Late & Early Going' else case when (da.m_time>bt1.in_time and da.m_time<>'TOUR' and da.m_time<>'COMPEN' and da.pay_id not in (50,52,7)) and da.e_time>=bt2.out_time then 'Late' else case when da.pay_id in (50) and da.E_TIME is null then  'REG-Morning & Non-Marking Evening'   else case when da.pay_id in (51) and da.M_TIME is null then 'REG-EVENING & Non-Marking Morning'  else  case  when da.pay_id in (50) and da.e_time<>'TOUR' and da.e_time<>'COMPEN'  and da.E_TIME <bt2.out_time  then 'REG-Morning & Early-Going'  else  case  when da.pay_id in (51) and da.m_time<>'TOUR' and da.m_time<>'COMPEN'  and da.M_TIME>bt1.in_time then  'REG-Evening & Late' else case when da.pay_id in (52) then 'REG-Morning & Evening'  else '-' end end end end end end end end end end end end end end end end as remarks,case when da.gun_status<>0 then 'PUNCHING-BLOCK' else '--' end as block,(select substr(ttb.in_time,1,5)||'-'||substr(ttb.out_time,1,5) from Time_Tab ttb where ttb.shift_id=da.shift_id)shifttime from ATTENDANCE da,employee_master em, time_tab bt1, time_tab bt2,employ_firm ef where  em.emp_code=da.emp_code and  ef.firm_id='" & firm & "'   and ef.emp_code=em.emp_code and da.curr_date between '" & fdt & "' and '" & tdt & "' and bt1.shift_id=da.m_shift and bt2.shift_id=da.e_shift and da.emp_code=" & emp & " order by day"
            'sql = "select curr_date as day, decode(da.m_time, NULL, '----------', da.m_time) as morning_time, decode(e_time, NULL, '----------', e_time) as evening_time, case when (da.m_time is null and da.pay_id not in (50, 52)) and (da.pay_id not in (51, 52) and da.e_time is null) then 'Absent' else case when da.pay_id in (50) and da.e_time is not null then 'Morning-REG' else case when da.pay_id in (51) and da.m_time is not null then 'Evening-REG' else case when da.pay_id in (52) then 'BOTH-REG' else case when (da.m_time > bt1.in_time and da.m_time <> 'TOUR' and da.m_time <> 'COMPEN' and da.pay_id not in (50, 7, 52)) and (da.e_time is null and da.pay_id not in (51, 7, 52)) then 'Late & Non-Marking' else case when da.m_time <= bt1.in_time and (da.e_time is null and da.pay_id not in (51, 52)) then case when da.PAY_ID in (7) then 'Absent' else 'Non-Marking Evening' end else case when (da.m_time is null and da.pay_id not in (50, 52, 7)) and da.e_time < bt2.out_time then 'Non-Marking Morning & Early-Going' else case when (da.m_time is null and da.pay_id not in (50, 52)) and da.e_time >= bt2.out_time then case when da.PAY_ID in (7) then 'Absent' else 'Non-Marking Morning' end else case when da.m_time <= bt1.in_time and (da.e_time < bt2.out_time and da.e_time <> 'TOUR' and da.e_time <> 'COMPEN' and da.pay_id not in (51, 52, 7)) then 'Early-Going' else case when (da.m_time > bt1.in_time and da.pay_id not in (50, 52)) and (da.e_time < bt2.out_time and da.pay_id not in (51, 52, 7)) then 'Late & Early Going' else case when (da.m_time > bt1.in_time and da.m_time <> 'TOUR' and da.m_time <> 'COMPEN' and da.pay_id not in (50, 52, 7)) and da.e_time >= bt2.out_time then 'Late' else case when da.pay_id in (50) and da.E_TIME is null then 'REG-Morning & Non-Marking Evening' else case when da.pay_id in (51) and da.M_TIME is null then 'REG-EVENING & Non-Marking Morning' else case when da.pay_id in (50) and da.e_time <> 'TOUR' and da.e_time <> 'COMPEN' and da.E_TIME < bt2.out_time then 'REG-Morning & Early-Going' else case when da.pay_id in (51) and da.m_time <> 'TOUR' and da.m_time <> 'COMPEN' and da.M_TIME > bt1.in_time then 'REG-Evening & Late' else case when da.pay_id in (52) then 'REG-Morning & Evening' else '-' end end end end end end end end end end end end end end end end as remarks, case when ((da.M_TIME is not null or da.E_TIME is not null) and (da.M_TIME not in ('COMPEN', 'TOUR')) and (da.gun_status <> 0)) then 'PUNCHING-BLOCK' else '--' end as block, (select substr(ttb.in_time, 1, 5) || '-' || substr(ttb.out_time, 1, 5) from MACTECH.Time_Tab ttb where ttb.shift_id = da.shift_id) shifttime from mactech.ATTENDANCE da, mactech.employee_master em, mactech.time_tab bt1, mactech.time_tab bt2, mactech.employ_firm ef where em.emp_code = da.emp_code and ef.firm_id = '" & firm & "' and ef.emp_code = em.emp_code and da.curr_date between '" & fdt & "' and '" & tdt & "' and bt1.shift_id = da.m_shift and bt2.shift_id = da.e_shift and da.emp_code = " & emp & " order by day "
            sql = "select curr_date as day, decode(da.m_time, NULL, '----------', da.m_time) as morning_time, decode(e_time, NULL, '----------', e_time) as evening_time, case when (da.m_time is null and da.pay_id not in (50, 52)) and (da.pay_id not in (51, 52) and da.e_time is null) then 'Absent' else case when da.pay_id in (50) and da.e_time is not null then 'Morning-REG' else case when da.pay_id in (51) and da.m_time is not null then 'Evening-REG' else case when da.pay_id in (52) then 'BOTH-REG' else case when (da.m_time > bt1.in_time and da.m_time <> 'TOUR' and da.m_time <> 'COMPEN' and da.pay_id not in (50, 7, 52)) and (da.e_time is null and da.pay_id not in (51, 7, 52)) then 'Late & Non-Marking' else case when da.m_time <= bt1.in_time and (da.e_time is null and da.pay_id not in (51, 52)) then case when da.PAY_ID in (7) then 'Absent' else 'Non-Marking Evening' end else case when (da.m_time is null and da.pay_id not in (50, 52, 7)) and da.e_time < bt2.out_time then CASE WHEN ac.status = 0 THEN 'Non-Marking Morning & Early-Going Applied' WHEN ac.status = 1 THEN 'Non-Marking Morning & Early-Going Sanctioned' ELSE 'Non-Marking Morning & Early-Going' END else case when (da.m_time is null and da.pay_id not in (50, 52)) and da.e_time >= bt2.out_time then case when da.PAY_ID in (7) then 'Absent' else 'Non-Marking Morning' end else case when da.m_time <= bt1.in_time and (da.e_time < bt2.out_time and da.e_time <> 'TOUR' and da.e_time <> 'COMPEN' and da.pay_id not in (51, 52, 7)) then CASE WHEN ac.status = 0 THEN 'Early-Going Applied' WHEN ac.status = 1 THEN 'Early-Going Sanctioned' ELSE 'Early-Going' END else case when (da.m_time > bt1.in_time and da.pay_id not in (50, 52)) and (da.e_time < bt2.out_time and da.pay_id not in (51, 52, 7)) then CASE WHEN ac.status = 0 THEN 'Late & Early-Going Applied' WHEN ac.status = 1 THEN 'Late & Early-Going Sanctioned' ELSE 'Late & Early-Going' END else case when (da.m_time > bt1.in_time and da.m_time <> 'TOUR' and da.m_time <> 'COMPEN' and da.pay_id not in (50, 52, 7)) and da.e_time >= bt2.out_time then 'Late' else case when da.pay_id in (50) and da.E_TIME is null then 'REG-Morning & Non-Marking Evening' else case when da.pay_id in (51) and da.M_TIME is null then 'REG-EVENING & Non-Marking Morning' else case when da.pay_id in (50) and da.e_time <> 'TOUR' and da.e_time <> 'COMPEN' and da.E_TIME < bt2.out_time then CASE WHEN ac.status = 0 THEN ' REG-Morning & Early-Going Applied' WHEN ac.status = 1 THEN 'REG-Morning & Early-Going Sanctioned' ELSE 'REG-Morning & Early-Going' END else case when da.pay_id in (51) and da.m_time <> 'TOUR' and da.m_time <> 'COMPEN' and da.M_TIME > bt1.in_time then 'REG-Evening & Late' else case when da.pay_id in (52) then 'REG-Morning & Evening' else '-' end end end end end end end end end end end end end end end end as remarks, case when ((da.M_TIME is not null or da.E_TIME is not null) and (da.M_TIME not in ('COMPEN', 'TOUR')) and (da.gun_status <> 0)) then 'PUNCHING-BLOCK' else '--' end as block, (select substr(ttb.in_time, 1, 5) || '-' || substr(ttb.out_time, 1, 5) from MACTECH.Time_Tab ttb where ttb.shift_id = da.shift_id) shifttime from mactech.ATTENDANCE da left join (select ab.emp_code, ab.status, ab.going_dt from mactech.hrm_earlygoing_appl ab where ab.status <> 2) ac on (da.EMP_CODE = ac.emp_code and ac.going_dt = da.CURR_DATE), mactech.employee_master em, mactech.time_tab bt1, mactech.time_tab bt2, mactech.employ_firm ef where em.emp_code = da.emp_code and ef.firm_id = '" & firm & "' and ef.emp_code = em.emp_code and da.curr_date between '" & fdt & "' and '" & tdt & "' and bt1.shift_id = da.m_shift and bt2.shift_id = da.e_shift and da.emp_code = " & emp & " order by day"
        Else
            ...
            ' sql = "select distinct curr_date as day, decode(da.m_time, NULL, '----------', da.m_time) as morning_time, decode(e_time, NULL, '----------', e_time) as evening_time, case when (da.m_time is null and da.pay_id not in (50, 52)) and (da.pay_id not in (51, 52) and da.e_time is null) then 'Absent' else case when da.pay_id in (50) and da.e_time is not null then 'Morning-REG' else case when da.pay_id in (51) and da.m_time is not null then 'Evening-REG' else case when da.pay_id in (52) then 'BOTH-REG' else case when (da.m_time > bt1.in_time and da.m_time <> 'TOUR' and da.m_time <> 'COMPEN' and da.pay_id not in (50, 7, 52)) and (da.e_time is null and da.pay_id not in (51, 7, 52)) then 'Late & Non-Marking' else case when da.m_time <= bt1.in_time and (da.e_time is null and da.pay_id not in (51, 52)) then 'Non-Marking Evening' else case when (da.m_time is null and da.pay_id not in (50, 52, 7)) and da.e_time < bt2.out_time then 'Non-Marking Morning & Early-Going' else case when (da.m_time is null and da.pay_id not in (50, 52)) and da.e_time >= bt2.out_time then 'Non-Marking Morning' else case when da.m_time <= bt1.in_time and (da.e_time < bt2.out_time and da.e_time <> 'TOUR' and da.e_time <> 'COMPEN' and da.pay_id not in (51, 52, 7)) then case when em.department_id in (748,825) and da.E_TIME>bt2.early_time and da.E_TIME<bt2.lmt_time then 'Second Half Leave' else 'Early-Going' end else case when (da.m_time > bt1.in_time and da.pay_id not in (50, 52)) and (da.e_time < bt2.out_time and da.pay_id not in (51, 52, 7)) then 'Late & Early Going' else case when (da.m_time > bt1.in_time and da.m_time <> 'TOUR' and da.m_time <> 'COMPEN' and da.pay_id not in (50, 52, 7)) and da.e_time >= bt2.out_time then case when em.department_id in (748,825) and da.M_TIME<bt2.early_time and da.E_TIME>bt2.lmt_time then 'First Half Leave' else 'Late' end else case when da.pay_id in (50) and da.E_TIME is null then 'REG-Morning & Non-Marking Evening' else case when da.pay_id in (51) and da.M_TIME is null then 'REG-EVENING & Non-Marking Morning' else case when da.pay_id in (50) and da.e_time <> 'TOUR' and da.e_time <> 'COMPEN' and da.E_TIME < bt2.out_time then 'REG-Morning & Early-Going' else case when da.pay_id in (51) and da.m_time <> 'TOUR' and da.m_time <> 'COMPEN' and da.M_TIME > bt1.in_time then 'REG-Evening & Late' else case when da.pay_id in (52) then 'REG-Morning & Evening' else '-' end end end end end end end end end end end end end end end end as remarks, case when da.gun_status <> 0 then 'PUNCHING-BLOCK' else '--' end as block from ATTENDANCE da, employee_master em, time_tab bt1, time_tab bt2, employ_firm ef where em.emp_code = da.emp_code and ef.firm_id = '" & firm & "' and ef.emp_code = em.emp_code and da.curr_date between '" & fdt & "' and '" & tdt & "'' and bt1.shift_id = da.m_shift and bt2.shift_id = da.e_shift and da.emp_code = " & emp & " order by day"
        End If
        ...
        If dt1.Rows.Count > 0 Then
            ...
            Dim td58a As New TableCell
            td58a.Attributes.Add("width", "25%")
            td58a.ColumnSpan = 15
            td58a.HorizontalAlign = HorizontalAlign.Center
            td58a.Text = "<font size=2.5><b>SHIFT</b></font>"
            tr5.Controls.Add(td58a)
            tb.Controls.Add(tr5)
            tb.Controls.Add(tr5)
            ...
            For Each dr In dt.Rows
                ...
                td67.Text = "<font size=2>" & dr(4) & "</font>"
                ...
                If Not IsDBNull(dr(3)) Then
                    ...
                    If dr(3) = "Absent" Or dr(3) = "-" Or dr(3) = "Non-Marking Morning" Or dr(3) = "Non-Marking Evening" Then
                        ...
                        dt6 = oh.ExecuteDataSet("select sum(c) from (select count(*) as c from hrm_leave_apply_sanction a where a.emp_code = " & emp & " and ((to_date('" & Format(dr(0), "dd/MMM/yyyy") & "') between to_date(a.leave_frdate) and to_date(a.leave_todate)) or (to_date('" & Format(dr(0), "dd/MMM/yyyy") & "') between to_date(a.leave_frdate) and to_date(a.leave_todate))) and a.status_id in (0, 4, 5, 1) union all select count(*) as c from tbl_lop_cancelled b where b.empcode = " & emp & " and ((to_date('" & Format(dr(0), "dd/MMM/yyyy") & "') between to_date(b.lopcancelltndate) and to_date(b.lopcancelltndate)) or (to_date('" & Format(dr(0), "dd/MMM/yyyy") & "') between to_date(b.lopcancelltndate) and to_date(b.lopcancelltndate))) and b.status in (0, 4, 5, 1))").Tables(0)
                        ...
                        'dt13 = oh.ExecuteDataSet("select count(*) from tbl_lop_cancelled a where a.empcode =" & emp & " and ((to_date('" & Format(dr(0), "dd/MMM/yyyy") & "') between to_date(a.lopcancelltndate) and to_date(a.lopcancelltndate))) and a.status in (0, 4, 5, 1)").Tables(0)


                        If dt6.Rows(0)(0) > 0 Or dt12.Rows(0)(0) > 0 Then
                            'dt7 = oh.ExecuteDataSet("select distinct a.emp_code, case when a.status_id = 0 then 'Leave applied' when a.status_id = 4 then 'Leave Recommended' when a.status_id = 5 then 'Leave Recommended' when a.status_id = 1 then 'Leave Sanctioned' when a.status_id = 2 then 'Leave Rejected' end as status from hrm_leave_apply_sanction a where a.emp_code = " & emp & " and ((to_date('" & Format(dr(0), "dd/MMM/yyyy") & "') between to_date(a.leave_frdate) and to_date(a.leave_todate)) or (to_date('" & Format(dr(0), "dd/MMM/yyyy") & "') between to_date(a.leave_frdate) and to_date(a.leave_todate))) union select distinct e.emp_code, case when (e.leave_id not in (4) and e.leave_process_id in (1, 2)) then 'Leave Sanctioned' end as status from employ_leave_dtl e where e.emp_code = " & emp & " and ((to_date('" & Format(dr(0), "dd/MMM/yyyy") & "') between to_date(e.leave_frdate) and to_date(e.leave_todate)) or (to_date('" & Format(dr(0), "dd/MMM/yyyy") & "') between to_date(e.leave_frdate) and to_date(e.leave_todate))) and e.leave_id not in (4) and e.leave_process_id in (1, 2) union select distinct t.empcode, case when t.status = 0 then 'Absent' when t.status = 4 then 'Absent' when t.status = 5 then 'Absent' when t.status = 1 then 'Lop cancelled - Sunday worked' when t.status = 2 then 'Absent' end as status from tbl_lop_cancelled t where t.empcode = " & emp & " and ((to_date('" & Format(dr(0), "dd/MMM/yyyy") & "') between to_date(t.lopcancelltndate) and to_date(t.lopcancelltndate))) union select distinct e.emp_code, case when (e.leave_id in (4) and e.leave_process_id in (0)) then 'Lop cancelled - Sunday worked' end as status from employ_leave_dtl e where e.emp_code = " & emp & " and ((to_date('" & Format(dr(0), "dd/MMM/yyyy") & "') between to_date(e.leave_frdate) and to_date(e.leave_todate)) or (to_date('" & Format(dr(0), "dd/MMM/yyyy") & "') between to_date(e.leave_frdate) and to_date(e.leave_todate))) and e.leave_id in (4) and e.cancel_reason = 'SUNDAY WORKING LOP CANCELATION' and e.leave_process_id in (0)").Tables(0)
                            dt7 = oh.ExecuteDataSet("select distinct a.emp_code, case when a.status_id = 0 then 'Leave applied' when a.status_id = 4 then 'Leave Recommended' when a.status_id = 5 then 'Leave Recommended' when a.status_id = 1 then 'Leave Sanctioned' when a.status_id = 2 then 'Leave Rejected' end as status from hrm_leave_apply_sanction a where a.emp_code = " & emp & " and ((to_date('" & Format(dr(0), "dd/MMM/yyyy") & "') between to_date(a.leave_frdate) and to_date(a.leave_todate)) or (to_date('" & Format(dr(0), "dd/MMM/yyyy") & "') between to_date(a.leave_frdate) and to_date(a.leave_todate))) AND a.leave_enter_date = (SELECT MAX(b.leave_enter_date) FROM hrm_leave_apply_sanction b WHERE b.emp_code = " & emp & " AND TO_DATE('" & Format(dr(0), "dd/MMM/yyyy") & "', 'dd-mon-yyyy') BETWEEN TO_DATE(b.leave_frdate, 'dd-mon-yyyy') AND TO_DATE(b.leave_todate, 'dd-mon-yyyy')) union select distinct e.emp_code, case when (e.leave_id not in (4) and e.leave_process_id in (1, 2)) then 'Leave Sanctioned' end as status from employ_leave_dtl e where e.emp_code = " & emp & " and ((to_date('" & Format(dr(0), "dd/MMM/yyyy") & "') between to_date(e.leave_frdate) and to_date(e.leave_todate)) or (to_date('" & Format(dr(0), "dd/MMM/yyyy") & "') between to_date(e.leave_frdate) and to_date(e.leave_todate))) and e.leave_id not in (4) and e.leave_process_id in (1, 2) union select distinct t.empcode, case when t.status = 0 then 'Absent' when t.status = 4 then 'Absent' when t.status = 5 then 'Absent' when t.status = 1 then 'Lop cancelled - Sunday worked' when t.status = 2 then 'Absent' end as status from tbl_lop_cancelled t where t.empcode = " & emp & " and ((to_date('" & Format(dr(0), "dd/MMM/yyyy") & "') between to_date(t.lopcancelltndate) and to_date(t.lopcancelltndate))) union select distinct e.emp_code, case when (e.leave_id in (4) and e.leave_process_id in (0)) then 'Lop cancelled - Sunday worked' end as status from employ_leave_dtl e where e.emp_code = " & emp & " and ((to_date('" & Format(dr(0), "dd/MMM/yyyy") & "') between to_date(e.leave_frdate) and to_date(e.leave_todate)) or (to_date('" & Format(dr(0), "dd/MMM/yyyy") & "') between to_date(e.leave_frdate) and to_date(e.leave_todate))) and e.leave_id in (4) and e.cancel_reason = 'SUNDAY WORKING LOP CANCELATION' and e.leave_process_id in (0)").Tables(0)
                ...
                Dim td68a As New TableCell
                td68a.Attributes.Add("width", "25%")
                td68a.ColumnSpan = 15
                td68a.HorizontalAlign = HorizontalAlign.Center
                td68a.Text = "<font size=2>" & dr(5) & "</font>"
                tr6.Controls.Add(td68a)
                tb.Controls.Add(tr6)
