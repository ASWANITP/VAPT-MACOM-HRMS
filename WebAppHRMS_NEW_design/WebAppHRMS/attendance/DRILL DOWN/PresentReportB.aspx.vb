...
Partial Class Attendence_Report_PresentReportB_0e8de44c7165
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Select Case category
            Case 1
                sql = "select count(da.emp_code),bm.branch_name,bm.branch_id from ATTENDANCE da,branch_master bm,AREA_DETAIL AD where bm.branch_id=da.branch_id  and da.branch_id=AD.BRANCH_ID AND AD.AREA_ID=" & areaid & " and da.curr_date>=to_date('" & fdate & "') and da.curr_date<=to_date('" & tdate & "') and (da.m_time is NOT NULL or da.e_time is NOT NULL or da.pay_id in (50,51,52,7)) and da.shift_id not in(4,5) and bm.firm_id = " & Session("firm_id") & " group by bm.branch_name,bm.branch_id"
                cat = "PRESENT"
            Case 2
                sql = "select count(da.emp_code),bm.branch_name,bm.branch_id from ATTENDANCE da,branch_master bm,AREA_DETAIL AD where bm.branch_id=da.branch_id  and da.branch_id=AD.BRANCH_ID AND AD.AREA_ID=" & areaid & " and da.curr_date>=to_date('" & fdate & "') and da.curr_date<=to_date('" & tdate & "') and da.m_time is NULL and da.e_time is NULL and da.pay_id not in (50,51,52,7)  and da.shift_id not in(4,5) and bm.firm_id = " & Session("firm_id") & " group by bm.branch_name,bm.branch_id"
                cat = "ABSENT"
            Case 3
                sql = "select count(da.emp_code),bm.branch_name,bm.branch_id from ATTENDANCE da,time_tab tt,branch_master bm,AREA_DETAIL AD where bm.branch_id=da.branch_id  and da.branch_id=AD.BRANCH_ID AND AD.AREA_ID=" & areaid & " and da.curr_date>=to_date('" & fdate & "') and da.curr_date<=to_date('" & tdate & "') and da.m_shift= tt.shift_id And da.m_time > tt.in_time and da.pay_id not in (50,52,7) and bm.firm_id = " & Session("firm_id") & "  group by bm.branch_name,bm.branch_id"
                cat = "LATE"
            Case 4
                sql = "select count(da.emp_code),bm.branch_name,bm.branch_id from ATTENDANCE da,time_tab tt,branch_master bm,AREA_DETAIL AD where bm.branch_id=da.branch_id  and da.branch_id=AD.BRANCH_ID AND AD.AREA_ID=" & areaid & " and da.curr_date>=to_date('" & fdate & "') and da.curr_date<=to_date('" & tdate & "') and da.e_shift= tt.shift_id And da.e_time < tt.out_time and da.pay_id not in (51,52,7) and bm.firm_id = " & Session("firm_id") & " group by bm.branch_name,bm.branch_id"
                cat = "EARLY GOING"
            Case 5
                sql = "select count(da.emp_code),bm.branch_name,bm.branch_id from ATTENDANCE da,branch_master bm,AREA_DETAIL AD where bm.branch_id=da.branch_id  and da.branch_id=AD.BRANCH_ID AND AD.AREA_ID=" & areaid & " and da.curr_date>=to_date('" & fdate & "') and da.curr_date<=to_date('" & tdate & "') and (((da.m_time is  null and da.pay_id<>50 ) or (da.e_time is  null and da.pay_id<>51)) AND NOT(DA.M_TIME IS NULL AND DA.E_TIME IS NULL) and da.pay_id not in (52)) and da.shift_id not in(4,5) and bm.firm_id = " & Session("firm_id") & " group by bm.branch_name,bm.branch_id"
                cat = "EARLY GOING"
        End Select
        ...
        Dim ss As String = oh.ExecuteDataSet("select area_name from area_master where area_id=" & Request.QueryString("areaid")).Tables(0).Rows(0)(0)
        ...
        td41.Text = "<font size=3><b>" & cat & " &nbsp Report From :&nbsp" & fdate & " &nbsp To :" & tdate & " Of Area " & ss & "</b></font>"
        ...
        For Each dr In dt.Rows
            ...
            sql1 = "select count(da.emp_code),da.BRANCH_ID from ATTENDANCE da,branch_master bm where bm.branch_id=da.branch_id  and da.branch_id=" & dr(2) & " and da.curr_date>=to_date('" & fdate & "') and da.curr_date<=to_date('" & tdate & "')  group by da.BRANCH_ID"
