...
Partial Class Attendence_Report_PresentReport_08710abd2175
    Inherits System.Web.UI.Page
    Dim dt, dt1 As New DataTable
    Dim oh As New Helper.Oracle.OracleHelper
    Dim sql, sql1 As String
    Dim dr As DataRow
    Dim lper, totalpres, totalabsent, totallate, totalearly, totalnon, totalstr, tlper As Double
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        sql = "select region_id,  reg_name,  count(emp_code) as Total,  sum(case  when (m_time is NOT NULL or e_time is NOT NULL) then  1  else  0  end) as present,  sum(case  when (m_time is NULL and e_time is NULL) then  1  else  0  end) as Absent,  sum(case  when (m_time > in_time) then  1  else  0  end) as Late,  sum(case  when (e_time < out_time) then  1  else  0  end) as EarlyGoing,  sum(case  when ((m_time is null and e_time is not null) OR  (M_TIME IS not NULL AND E_TIME IS NULL and  CURR_DATE < to_date(sysdate))) then  1  else  0  end) as NonMarking  from (select da.*  from attendance_detail da,branch_master b  where da.CURR_DATE >= to_date('" & frdate & "')  and da.curr_date <= to_date('" & frdate & "')  and da.BRANCH_ID=b.branch_id  and b.firm_id=" & Session("firm_id") & "  and da.shift_id not in (4, 5)) t  left outer join (select a.region_id,  a.reg_name,  count(a.emp_code) as Total,  sum(case  when (a.m_time is NOT NULL or a.e_time is NOT NULL) then  1  else  0  end) as present,  sum(case  when (a.m_time is NULL and a.e_time is NULL) then  1  else  0  end) as Absent,  sum(case  when (a.m_time > in_time) then  1  else  0  end) as Late,  sum(case  when (a.e_time < out_time) then  1  else  0  end) as EarlyGoing,  sum(case  when ((a.m_time is null and a.e_time is not null) OR  (a.M_TIME IS not NULL AND a.E_TIME IS NULL and  CURR_DATE < to_date(sysdate))) then  1  else  0  end) as NonMarking  from attendance_detail a,branch_master b  where  a.CURR_DATE >= to_date('" & frdate & "')  and  a.curr_date <= to_date('" & frdate & "')  and  a.shift_id not in (4, 5)  and a.branch_id=b.branch_id  and b.firm_id=" & Session("firm_id") & "  group by a.region_id, a.reg_name)  using (region_id, reg_name)  group by region_id, reg_name"
