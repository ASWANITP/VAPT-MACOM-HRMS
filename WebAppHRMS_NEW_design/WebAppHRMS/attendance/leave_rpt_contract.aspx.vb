...
Partial Class attendance_leave_rpt_contract_77ceda449979
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Me.oldCnt = oh.ExecuteDataSet("select count(*) from employee_master_dtl where new_empcode = 6480").Tables(0).Rows(0)(0)
        If oldCnt = 1 Then
            Me.oldCode = oh.ExecuteDataSet("select emp_code from employee_master_dtl where new_empcode = 6480").Tables(0).Rows(0)(0)
        Else
            Me.oldCode = 0
        End If
        Dim name As String = "select a.leave_frdate,a.leave_todate,case when a.leave_form in(11,12) then to_number(0.5) else a.leave_days end leave_days,b.leave_abbr,decode(a.leave_reason,Null,'----',a.leave_reason) from employ_leave_dtl a,leave_master b where a.leave_id=b.leave_id and a.leave_process_id not in (0,3) and a.status=1 and a.emp_code in (6480) and to_date(leave_frdate)>='" & Request.QueryString.Get("fdt") & "' and to_date(leave_todate)<='" & Request.QueryString.Get("tdt") & "' order by to_date(leave_frdate)"
        ...
        If Session("firm_id") = 28 Then
            ...
            dt1 = oh.ExecuteDataSet("select a.emp_code, a.emp_name, a.join_dt, case when b.designation_id <> 7 then b.designation || '/' || b.ctgry || '/' || b.ctgry_code when b.designation_id = 7 and exists (select t.emp_code, max(t.year_pass), t.qualification, qc.category_id from employ_qualification_dtl t, qualification_master qm, qualification_category qc where t.emp_code = a.emp_code and qm.qualification_id = t.qualification and qc.category_id = qm.category_id and qc.category_id IN (4, 3) group by t.emp_code, t.qualification, qc.category_id) then b.designation || '/' || 'JR. MANAGEMENT' || '/' || 'JM 4' when b.designation_id = 7 and exists (select t.emp_code, max(t.year_pass), t.qualification, qc.category_id from employ_qualification_dtl t, qualification_master qm, qualification_category qc where t.emp_code = a.emp_code and qm.qualification_id = t.qualification and qc.category_id = qm.category_id and qc.category_id IN (7) group by t.emp_code, t.qualification, qc.category_id) then b.designation || '/' || 'STAFF' || '/' || 'JM 2' when b.designation_id = 7 and exists (select t.emp_code, max(t.year_pass), t.qualification, qc.category_id from employ_qualification_dtl t, qualification_master qm, qualification_category qc where t.emp_code = a.emp_code and qm.qualification_id = t.qualification and qc.category_id = qm.category_id and qc.category_id IN (5, 6, 8) group by t.emp_code, t.qualification, qc.category_id) then b.designation || '/' || 'STAFF' || '/' || 'JM 3' when b.designation_id = 7 and exists (select t.emp_code, max(t.year_pass), t.qualification, qc.category_id from employ_qualification_dtl t, qualification_master qm, qualification_category qc where t.emp_code = a.emp_code and qm.qualification_id = t.qualification and qc.category_id = qm.category_id and qc.category_id IN (2) group by t.emp_code, t.qualification, qc.category_id) then b.designation || '/' || 'JR. MANAGEMENT' || '/' || 'JM 5' else b.designation || '/' || '-----' || '/' || '---' end, c.dep_name, d.branch_name from employee_master a, designation_master b, department_mst c, branch_master_all d where a.designation_id = b.designation_id and a.department_id = c.dep_id and a.branch_id = d.branch_id and a.emp_code = 6480 union select a.emp_code, a.emp_name, a.join_dt, case when b.designation_id <> 7 then b.designation || '/' || b.ctgry || '/' || b.ctgry_code when b.designation_id = 7 and exists (select t.emp_code, max(t.year_pass), t.qualification, qc.category_id from employ_qualification_dtl t, qualification_master qm, qualification_category qc where t.emp_code = a.emp_code and qm.qualification_id = t.qualification and qc.category_id = qm.category_id and qc.category_id IN (4, 3) group by t.emp_code, t.qualification, qc.category_id) then b.designation || '/' || 'JR. MANAGEMENT' || '/' || 'JM 4' when b.designation_id = 7 and exists (select t.emp_code, max(t.year_pass), t.qualification, qc.category_id from employ_qualification_dtl t, qualification_master qm, qualification_category qc where t.emp_code = a.emp_code and qm.qualification_id = t.qualification and qc.category_id = qm.category_id and qc.category_id IN (7) group by t.emp_code, t.qualification, qc.category_id) then b.designation || '/' || 'STAFF' || '/' || 'JM 2' when b.designation_id = 7 and exists (select t.emp_code, max(t.year_pass), t.qualification, qc.category_id from employ_qualification_dtl t, qualification_master qm, qualification_category qc where t.emp_code = a.emp_code and qm.qualification_id = t.qualification and qc.category_id = qm.category_id and qc.category_id IN (5, 6, 8) group by t.emp_code, t.qualification, qc.category_id) then b.designation || '/' || 'STAFF' || '/' || 'JM 3' when b.designation_id = 7 and exists (select t.emp_code, max(t.year_pass), t.qualification, qc.category_id from employ_qualification_dtl t, qualification_master qm, qualification_category qc where t.emp_code = a.emp_code and qm.qualification_id = t.qualification and qc.category_id = qm.category_id and qc.category_id IN (2) group by t.emp_code, t.qualification, qc.category_id) then b.designation || '/' || 'JR. MANAGEMENT' || '/' || 'JM 5' else b.designation || '/' || '-----' || '/' || '---' end, c.dep_name, d.branch_name from employee_master a, designation_master b, department_mst c, before_completion d where a.designation_id = b.designation_id and a.department_id = c.dep_id and a.branch_id = d.old_id and d.branch_id is null and a.emp_code = 6480").Tables(0)

        Else
            dt1 = oh.ExecuteDataSet("select a.emp_code,a.emp_name,a.join_dt,b.designation,c.dep_name ,d.branch_name from employee_master a,designation_master b,department_mst c,branch_master_all d where a.designation_id=b.designation_id and a.department_id=c.dep_id and a.branch_id=d.branch_id and a.emp_code=6480 union select a.emp_code,a.emp_name,a.join_dt,b.designation,c.dep_name ,d.branch_name from employee_master a,designation_master b,department_mst c,before_completion d where a.designation_id=b.designation_id and a.department_id=c.dep_id and a.branch_id=d.old_id and d.branch_id is null and a.emp_code=6480").Tables(0)
        End If
        pq = oh.ExecuteDataSet("select t.*  from emp_new_old_all t where T.new_CODE=6480 ").Tables(0)
