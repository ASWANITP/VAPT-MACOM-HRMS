...
Partial Class salary_hrm_salary_confirmation_77201fe81537
    ...
    Private Sub departmentfill()
        Dim dt As DataTable
        dt = oh.ExecuteDataSet("select distinct (d.dep_id), d.dep_name from department_mst d,hrm_bank_confirmation b,employee_master e where d.dep_id=e.department_id and b.emp_code=e.emp_code and e.branch_id=0 and e.status_id=1 and not exists (select emp_code from hrm_sd_confirmation where emp_code = e.EMP_CODE) order by d.dep_name").Tables(0)
        Try
    ...
    Public Sub RaiseCallbackEvent(ByVal eventArgument As String) Implements System.Web.UI.ICallbackEventHandler.RaiseCallbackEvent
        ...
        Select Case (DataStr(1))
            Case 1
                ...
                dt1 = oh.ExecuteDataSet("select h.emp_code,e.emp_name,h.acc_no,h.salary,decode(h.allowance,1,'BONUS',0,'EXGRATIA'),h.old_code from hrm_bank_confirmation h,employee_master e where h.emp_code=e.emp_code and e.status_id=1 and e.department_id=" & CODE & " and e.branch_id=0 and h.emp_code not in (select s.emp_code from hrm_sd_confirmation s)").Tables(0)
            ...
            Case 2
                ...
                Try
                    ...
                    oh.ExecuteNonQuery("hrmbonusbankconfirmation", p)
                    CbResult = p(3).Value
                Catch ex As Exception
                    CbResult = ex.Message
                End Try
