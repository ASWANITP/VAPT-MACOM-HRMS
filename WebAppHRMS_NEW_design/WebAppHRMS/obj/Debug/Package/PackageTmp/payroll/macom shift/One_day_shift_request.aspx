<%@ Page Language="VB" MasterPageFile="~/edp.master" AutoEventWireup="false" CodeBehind="One_day_shift_request.aspx.vb" Inherits="WebAppHRMS.One_day_shift_request" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="cph_edp" runat="server">
    ...
    <asp:ScriptManager ID="ScriptManager1" runat="server" />
     ...
     <script type="text/javascript">
         function checkShiftDate() {
             debugger;
             const input = document.getElementById('<%= txtShiftChangeDate.ClientID %>');
     
             const fromdt = input.value;
             const fromDate = new Date(fromdt);
             const today = new Date();


             if (fromDate < today) {
                 alert("ONLY FUTURE DATES ALLOWED" );
                 input.value = "";
                 return false;
             }
    ...
    window.onload = function () {
        document.getElementById('<%= txtShiftChangeDate.ClientID %>').onchange = checkShiftDate;
         };
     </script>
    ...
    <asp:DropDownList ID="ddlShiftSelection" runat="server" CssClass="scrTextBox" AutoPostBack="True">
        ...
        <div class="datePickerWrapper">
           <label for="txtShiftChangeDate">Date of Shift Change</label>
               <asp:TextBox ID="txtShiftChangeDate" runat="server" CssClass="scrTextBox" />
