...
Imports System.Net
Imports System.IO
Partial Class Employee_Punching_Movement_Approval_Macom_242346791301
    Inherits System.Web.UI.Page
    Dim oh As New Helper.Oracle.OracleHelper
    Dim dt, dt1, dt2, dtj, ceo, depp, dtr, dth, dap As New DataTable
    Dim sf(), frm As String
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        frm = Session("firm_id")
        If frm = 27 Then
            Response.Redirect("Movement_Approval_Mafarm.aspx")
            Exit Sub
        End If
        ...
        dth = oh.ExecuteDataSet("select s.post_id from employee_master s where s.emp_code=" & User(0) & "").Tables(0)
        If Not IsPostBack Then
            dt1 = oh.ExecuteDataSet("select count(*) from form_accessibility s where s.form_id=6001 and s.emp_id=" & dth.Rows(0)(0) & "").Tables(0)

            
            'dt1 = oh.ExecuteDataSet("select count(*) from mactech.tbl_movement_mst t where t.aprv_usr=" & User(0) & "").Tables(0)
            If dt1.Rows(0)(0) > 0 Then
                dt = oh.ExecuteDataSet("select '----SELECT----', '0' as empcode from dual union select e.emp_code || '-----' || e.emp_name || '-----' || to_char(a.reqst_dt)|| '-----' ||a.mov_id, e.emp_code || '-----' || e.emp_name || '*' || d.dep_name || '*' || ds.designation || '*' || p.post_name || '*' || b.branch_name || '*' || to_char(a.reqst_dt)  || '*' ||a.exit_time || '*' || a.entry_time || '*' || a.place || '*' || a.purpose || '*' ||a.mov_type from TBL_MOVEMENT_MST a, employee_master e, department_mst d, designation_master ds, post_mst p, branch b where a.emp_code = e.emp_code  and to_date(a.reqst_dt)=to_date(sysdate) and a.status_id = 1 and e.department_id = d.dep_id and e.designation_id = ds.designation_id and e.post_id = p.post_id and e.branch_id = b.BRANCH_ID and a.aprv_usr =" & User(0) & "").Tables(0)
                ...
                Else
                    ...
                    cl_script1.Append(" window.open('Movement_Approval_Macom.aspx','_self');")
    ...
    Protected Sub cmd_confirm_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmd_confirm.Click
        ...
        Try
            ...
            Dim sq178 As String = "UPDATE mactech.tbl_movement_mst el set el.approve_by=:san_emp_code,el.approve_dt=:recdt,el.status_id=:stat where el.emp_code=:code and to_char(el.reqst_dt)=:go_dt and el.mov_id=:movid"
            ...
            If oh.ExecuteNonQuery(sq178, prrr) Then
                ...
                '            reqstring = wgate.Rows(0)(0).ToString.Split("@")(1).Replace("mycode", arr1(0))
                ...
                cl_scrip1.Append("window.open('Movement_Approval_Macom.aspx','_self');")
    ...
    Protected Sub New_Reject_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles New_Reject.Click
        ...
        Else
            ...
            Try
                ...
                Dim sq178 As String = "UPDATE mactech.tbl_movement_mst el set el.approve_by=:rej_emp_code,el.approve_dt=:recdt,el.rej_reason=:rejrea,el.status_id=:stat,el.tot_time = 0 where el.emp_code=:code and to_char(el.reqst_dt)=:go_dt and el.mov_id=:movid "
